...
class BubbleBlackjackGame(tk.Frame):
    ...
    def __init__(self, parent, balance=10000, user=None, on_back=None,
        ...
        import uuid
        self.csm_game_id = f"Premiere_Blackjack:{self.username or 'local'}:{uuid.uuid4().hex}"
    ...
    def _maybe_settle_crazy7(self):
        ...
        c2 = seq[2]
        if c2[1] != '7':
            self._credit_side_bet(key, 5, '第一目标7')
            self.save_balance()
            return
    ...
    def show_game_instructions(self):
        ...
        rules=make_card('01  游戏玩法与连续洗牌说明','8副完整牌 · ENHC · H17')
        ...
        add_rule(right,'① 连续洗牌机','使用8副完整52张牌，共416张；所有牌局均由CSM_Shuffler连续洗牌机发牌。','#7cc8aa')
        add_rule(right,'② 连续发牌','牌张跨局保留在连续洗牌系统中，不建立或切换独立牌靴。','#7cc8aa')
        add_rule(right,'③ 牌张回收','每局结束后，已发出的牌进入弃牌区，由连续洗牌机按规则重新混入牌仓。','#7cc8aa')
        add_rule(right,'④ 无需换靴','不使用剩余牌阈值、切牌或烧牌流程；下一局直接由连续洗牌机供牌。','#7cc8aa')
        ...
        blocks=[
            ...
            ('疯狂7 · 仅主注-玩家',[('目标三张同花7','2000:1'),('目标三张7','500:1'),('前两目标同花7','150:1'),('前两目标7','25:1'),('第一目标7（第0张）','5:1')]),
