...
# Blackjack Classic + Main-Side bonus — 10% main-wager fee
import json
import math
import os
import random
...
BLACKJACK_VERSION = "V32-CSM-GAP-ROLL-REWARD-FLASH"
...
class BubbleBlackjackGame(tk.Frame):
    ...
    MAIN_FEE_RATE = 0.10        # charged once, on the original MAIN wager only
    MAIN_SIDE_RATES = ((0.25, 0.60), (0.50, 0.25), (0.75, 0.10), (1.00, 0.05))
    ...
    PHASE_BETTING = '投注叠堆黑杰克 · 请下注'
    PHASE_INSURANCE = '投注叠堆黑杰克 · 保险？'
    PHASE_EVEN_MONEY = '投注叠堆黑杰克 · 立刻获胜？'
    PHASE_PLAYING = '投注叠堆黑杰克 · 游戏中'
    PHASE_SETTLING = '投注叠堆黑杰克 · 结算中'
    ...
    def __init__(self, parent, balance=10000, user=None, on_back=None,
        ...
        self.round_main_fee = 0.0
        self.main_fee_collecting = False
        self.main_side_rank = None
        self.main_side_rate = 0.0
        self.main_side_match_count = 0
        self.main_side_resolved = False
        self.main_side_roll_running = False
        self.main_side_feature_visible = False
        ...
        self.external_golden_card_images = {}
        self.external_golden_card_pil = {}
    ...
    def load_original_card_assets(self):
        ...
        if directory:
            try:
                ...
                golden_directory = os.path.join(directory, 'Golden')
                if os.path.isdir(golden_directory):
                    for suit in BlackjackEngine.SUITS:
                        for rank in BlackjackEngine.RANKS:
                            path = os.path.join(golden_directory, f'{suit}{rank}.png')
                            if not os.path.exists(path):
                                continue
                            base = Image.open(path).convert('RGBA').resize((100, 140), resample)
                            self.external_golden_card_pil[(suit, rank)] = base
                            self.external_golden_card_images[(suit, rank)] = ImageTk.PhotoImage(base, master=self)
                ...
                self.external_golden_card_images.clear()
                self.external_golden_card_pil.clear()
                self.external_back_image = None
                self.external_back_pil = None
    ...
    def draw_animation_panel(self):
        ...
        self.rule_strip_text = c.create_text(
            (rx0 + rx1) / 2, (ry0 + ry1) / 2,
            text='庄家任何17点停牌 · 黑杰克3:2 · 主注手续费10% · 保险2:1',
    ...
    def draw_board(self):
        ...
        expanded_round_layout = dict(round_layout)
        expanded_round_layout['MAIN'] = (
            (480.0, 452.0, 720.0, 598.0), (600.0, 480.0), (600.0, 542.0))
        expanded_round_layout['Open Card Luck'] = (
            (269.0, 504.0, 399.0, 586.0), (334.0, 525.0), (334.0, 560.0))
        self.bet_layout_home = home
        self.bet_layout_round = expanded_round_layout
        self.bet_layout_round_restored = round_layout
        self.bet_layout_mode = 'home'
    ...
    def _animate_fee_chip_to_area(self, amount, origin, on_complete=None):
        """Fly the quoted 10% fee from the same origin as its MAIN chip."""
        amount = float(amount)
        if amount <= 0.0:
            if callable(on_complete): on_complete()
            return
        ...
        tx, ty = x1 - 18.0, y0 + 20.0
        items = self._draw_baccarat_chip(
            sx, sy, amount, tags=('fee_bet_fly',), radius=18,
            force_color=self.bet_chip_color(amount))
        self.canvas.tag_raise('fee_bet_fly')
        steps, frame_ms = 10, 20
        def frame(step=1):
            ratio = min(1.0, step / float(steps))
            eased = 1.0 - (1.0 - ratio) ** 2
            cx = sx + (tx - sx) * eased; cy = sy + (ty - sy) * eased; r = 18
            self.canvas.coords(items[0], cx-r, cy-r, cx+r, cy+r)
            self.canvas.coords(items[1], cx-r+3, cy-r+3, cx+r-3, cy+r-3)
            self.canvas.coords(items[2], cx, cy)
            if step < steps:
    ...
    def _amount_chip_origin(self, amount):
        ...
        return self.chip_selector_centers.get(source_value, (575.0, 674.0))
    ...
    def _animate_amount_chip_to_spot(self, spot_key, amount, on_complete=None):
        ...
        sx, sy = self._amount_chip_origin(amount)
        tx, ty = spot['chip_pos']
        items = self._draw_baccarat_chip(
    ...
    def update_bet_chips(self):
        ...
        # The mandatory fee is quoted beside MAIN while betting.  It is not
        # charged on any side bet, split wager, Double wager or insurance.
        if (not self.round_active and not self.main_fee_collecting
                and self.current_bet > 0.0):
            x0, y0, x1, _y1 = self.bet_spots['MAIN']['bounds']
            fx, fy = x1 - 18.0, y0 + 20.0
            visible_fee = max(0.0, self.current_bet * self.MAIN_FEE_RATE
                              - self.pending_fee_animation)
            if visible_fee > 0.0:
                self._draw_baccarat_chip(
                    fx, fy, visible_fee,
                    tags=('bet_chip_dynamic', 'main_fee_chip'), radius=18)
                self.canvas.create_text(
                    fx, fy - 27, text='10% FEE', font=(self.cn_font, 8, 'bold'),
                    fill='#ffcf68', tags=('bet_chip_dynamic', 'main_fee_chip'))
    ...
    def _finish_fee_bet_animation(self, amount):
        self.pending_fee_animation = max(
            0.0, self.pending_fee_animation - float(amount))
        self.bet_chip_animation_count = max(0, self.bet_chip_animation_count - 1)
        self.update_display()
    ...
    def _settlement_return_chip_sources(self):
        ...
        for idx,hand in enumerate(self.hands):
            ...
            bonus_count=int(hand.get('main_side_count',0))
            bonus_return=float(hand.get('main_side_return',0.0))
            if bonus_count>0 and bonus_return>0.0:
                each=bonus_return/bonus_count
                for x,y in self._main_side_token_positions(layout[idx]['base'],bonus_count):
                    sources.append((x,y,each,17))
    ...
    def place_bet(self):
        ...
        fee_amount = amount * self.MAIN_FEE_RATE
        self.pending_fee_animation += fee_amount
        self.bet_chip_animation_count += 2
        self.update_display()
        origin = self._selected_chip_origin()
        self._animate_selected_chip_to_spot(
            'MAIN', lambda: self._finish_bet_chip_animation('MAIN', amount), amount=amount)
        self._animate_fee_chip_to_area(
            fee_amount, origin, lambda:self._finish_fee_bet_animation(fee_amount))
    ...
    def repeat_last_bet(self):
        ...
        for key, add in additions.items():
            ...
            if key == 'MAIN':
                ...
                fee_add = add * self.MAIN_FEE_RATE
                self.pending_fee_animation += fee_add
            else:
        ...
        for key, add in additions.items():
            if add <= 0.0:
                continue
            origin = self._amount_chip_origin(add)
            ...
            if key == 'MAIN':
                ...
                self._animate_fee_chip_to_area(
                    fee_add, origin,
                    lambda a=fee_add:self._finish_fee_bet_animation(a))
    ...
    def _face_photo(self, card, golden=False):
        if golden:
            photo = self.external_golden_card_images.get(tuple(card))
            if photo is not None:
                return photo
        return self.external_card_images.get(tuple(card))

    def _create_scaled_flip_image(self, card, width, height, use_back=False, golden=False):
        if Image is None or ImageTk is None:
            return self.external_back_image if use_back else self._face_photo(card, golden=golden)
        if use_back:
            base = self.external_back_pil
        elif golden:
            base = self.external_golden_card_pil.get(tuple(card))
            if base is None:
    ...
    def _animate_card_from_shoe(self, card, target_x, target_y, on_complete=None,
                                flip=True, tag='round_animation_card', golden=False):
        ...
        def flip_step(step=0):
            ...
            if step > steps:
                if self.canvas.type(card_id) == 'image':
                    final = self._face_photo(card, golden=golden)
            ...
            if self.canvas.type(card_id) == 'image':
                image = self._create_scaled_flip_image(
                    card, width, 140, use_back=use_back, golden=golden)
    ...
    def _main_side_token_positions(self, base_position, count):
        """Place one/two awarded chips immediately left of the Original Bet."""
        bx, by = base_position
        if int(count) <= 1:
            return [(bx - 48.0, by)]
        return [(bx - 48.0, by - 17.0), (bx - 48.0, by + 17.0)]

    def _draw_percentage_chip(self, cx, cy, rate, tags, radius=18):
        """Draw the awarded chip as a gold puck with a black percentage."""
        r = float(radius)
        outer = self.canvas.create_oval(
            cx-r, cy-r, cx+r, cy+r, fill='#d8ae32', outline='#fff0a0', width=2,
            tags=tags)
        inner = self.canvas.create_oval(
            cx-r+4, cy-r+4, cx+r-4, cy+r-4, fill='#f0cb55',
            outline='#9a7215', width=1, tags=tags)
        label = self.canvas.create_text(
            cx, cy, text=f'{int(round(float(rate) * 100))}%',
            font=(self.cn_font, 9 if r >= 17 else 8, 'bold'), fill='#111111',
            tags=tags)
        return outer, inner, label

    def _animate_main_side_roll(self, on_complete=None):
        """Run the predetermined vertical-rank/horizontal-percentage reveal."""
        self.main_side_roll_running = True
        ranks = BlackjackEngine.RANKS
        rates = tuple(rate for rate, _prob in self.MAIN_SIDE_RATES)
        # The reveal lives entirely in the temporary gap between Open Card Luck
        # (right edge 399) and MAIN (left edge 480), never in the player zone.
        cx, card_top, card_w, card_h = 439.5, 428.0, 54.0, 76.0
        card_left = cx - card_w / 2.0
        rank_center_y = card_top + card_h / 2.0
        rail_left, rail_right, rail_y = 402.0, 477.0, 526.0
        total_steps = 190  # 190 × 20 ms = 3.8 seconds

        def draw_frame(step):
            self.canvas.delete('main_side_roll')
            self.canvas.create_rectangle(
                card_left, card_top, card_left + card_w, card_top + card_h,
                fill='#f4efe4', outline='#f4d65b', width=3,
                tags=('main_side_roll',))
            self.canvas.create_rectangle(
                rail_left, rail_y - 27.0, rail_right, rail_y + 27.0,
                fill='#152c29', outline='#f4d65b', width=2,
                tags=('main_side_roll',))

            if step < 166:
                phase = step / 5.0
                base_index = int(phase)
                offset_y = (phase - base_index) * 34.0
                rank_entries = (
                    (ranks[base_index % len(ranks)], rank_center_y + offset_y),
                    (ranks[(base_index + 1) % len(ranks)], rank_center_y - 34.0 + offset_y),
                )
            else:
                settle = min(1.0, (step - 166) / 24.0)
                wobble = (1.0 - settle) * 10.0 * math.sin(settle * math.pi * 4.0)
                rank_entries = ((self.main_side_rank, rank_center_y + wobble),)
            for rank, yy in rank_entries:
                if card_top + 15.0 <= yy <= card_top + card_h - 15.0:
                    self.canvas.create_text(
                        cx, yy, text=rank, font=('Arial', 21, 'bold'),
                        fill='#111111', tags=('main_side_roll',))

            if step < 166:
                phase_x = step / 7.0
                base_rate = int(phase_x)
                offset_x = (phase_x - base_rate) * 36.0
                for slot in range(-2, 3):
                    xx = cx + slot * 36.0 + offset_x
                    if rail_left + 13.0 <= xx <= rail_right - 13.0:
                        rate = rates[(base_rate + slot) % len(rates)]
                        self._draw_percentage_chip(
                            xx, rail_y, rate, tags=('main_side_roll',), radius=13)
            else:
                settle = min(1.0, (step - 166) / 24.0)
                wobble = (1.0 - settle) * 14.0 * math.sin(settle * math.pi * 4.0)
                self._draw_percentage_chip(
                    cx + wobble, rail_y, self.main_side_rate,
                    tags=('main_side_roll',), radius=15)

            self.canvas.tag_raise('main_side_roll')
            if step < total_steps:
                self._queue(20, draw_frame, step + 1)
            else:
                self.canvas.delete('main_side_roll')
                self.main_side_roll_running = False
                self.main_side_feature_visible = True
                self.render_cards(); self.update_display()
                if callable(on_complete):
                    on_complete()
    ...
    def _draw_main_side_feature(self):
        """Draw this round's sampled rank card and its still-pending chip."""
        self.canvas.delete('main_side_feature')
        if (not self.round_active or not self.main_side_rank
                or not self.main_side_feature_visible
                or self.main_side_roll_running):
            return
        spot = self.bet_spots.get('MAIN')
        if not spot:
            return
        x0, y0, _x1, _y1 = spot['bounds']
        x, y, w, h = x0 - 68.0, y0 - 24.0, 54.0, 76.0
        self.canvas.create_rectangle(
            x, y, x + w, y + h, fill='#f4efe4', outline='#f4d65b', width=2,
            tags=('main_side_feature',))
        self.canvas.create_text(
            x + w / 2.0, y + h / 2.0, text=self.main_side_rank,
            font=('Arial', 21, 'bold'), fill='#111111', justify='center',
            tags=('main_side_feature',))
        if not self.main_side_resolved:
            self._draw_percentage_chip(
                x + w / 2.0, y + h + 12.0, self.main_side_rate,
                tags=('main_side_feature',), radius=17)
        self.canvas.tag_raise('main_side_feature')

    def _animate_reward_chips_to_main(self, count, on_complete=None):
        """Move one/two cloned percentage chips to Original Bet simultaneously."""
        count = max(0, min(2, int(count)))
        if count <= 0:
            if callable(on_complete): on_complete()
            return
        spot = self.bet_spots['MAIN']
        x0, y0, _x1, _y1 = spot['bounds']
        source = (x0 - 41.0, y0 + 64.0)
        base = self._main_split_wager_layout(1)[0]['base']
        targets = self._main_side_token_positions(base, count)
        self.canvas.delete('main_side_transfer')
        moving = []
        for index, target in enumerate(targets):
            tag = f'main_side_transfer_{index}'
            self._draw_percentage_chip(
                source[0], source[1], self.main_side_rate,
                tags=('main_side_transfer', tag), radius=17)
            moving.append((tag, source[0], source[1], target[0], target[1]))
        self.canvas.tag_raise('main_side_transfer')
        previous = [0.0]
        steps, frame_ms = 22, 20
        def frame(step=1):
            t = min(1.0, step / float(steps)); eased = 1.0 - (1.0 - t) ** 3
            delta = eased - previous[0]; previous[0] = eased
            for tag, sx, sy, tx, ty in moving:
                self.canvas.move(tag, (tx - sx) * delta, (ty - sy) * delta)
            if step < steps:
                self._queue(frame_ms, frame, step + 1)
            else:
                self.canvas.delete('main_side_transfer')
    ...
    def _animate_reward_gap_closed(self, on_complete=None):
        """Return MAIN/Open Card Luck horizontally while keeping round Y positions."""
        keys = ('MAIN', 'Open Card Luck')
        starts = {
            key: (tuple(self.bet_spots[key]['bounds']),
                  tuple(self.bet_spots[key]['label_pos']),
                  tuple(self.bet_spots[key]['chip_pos']))
            for key in keys
        }
        target = self.bet_layout_round_restored
        steps, frame_ms = 12, 20
        def frame(step=1):
            t = min(1.0, step / float(steps)); eased = t*t*(3.0-2.0*t)
            for key in keys:
                spot = self.bet_spots[key]
                sb, sl, sc = starts[key]; tb, tl, tc = target[key]
                bounds = tuple(sb[i] + (tb[i]-sb[i])*eased for i in range(4))
                label_pos = tuple(sl[i] + (tl[i]-sl[i])*eased for i in range(2))
                chip_pos = tuple(sc[i] + (tc[i]-sc[i])*eased for i in range(2))
                self.canvas.coords(spot['rect'], *bounds)
                self.canvas.coords(spot['label'], *label_pos)
                spot['bounds'], spot['label_pos'], spot['chip_pos'] = bounds, label_pos, chip_pos
            self.bet_spot_bounds = self.bet_spots['MAIN']['bounds']
            self.update_bet_chips()
            self.canvas.delete('hand_wager'); self._draw_main_hand_wagers()
            self.canvas.tag_raise('dealt_card'); self.canvas.tag_raise('hand_wager')
            if step < steps:
                self._queue(frame_ms, frame, step + 1)
            else:
                self.bet_layout_mode = 'round'
                if callable(on_complete): on_complete()
        self._queue(frame_ms, frame, 1)
    ...
    def _draw_main_hand_wagers(self):
        ...
        for idx, hand in enumerate(self.hands):
            ...
            bonus_count = int(hand.get('main_side_count', 0))
            bonus_return = float(hand.get('main_side_return', 0.0))
            show_bonus = bonus_count > 0 and (
                not self.settlement_running or bonus_return > 0.0)
            if show_bonus:
                original_bonus_amount = self.round_original_bet * self.main_side_rate
                if self.settlement_running and self.flash_mode == 'win':
                    bonus_display = bonus_return / bonus_count
                else:
                    bonus_display = original_bonus_amount
                for _token_index, (tx, ty) in enumerate(
                        self._main_side_token_positions(pos['base'], bonus_count)):
                    if self.settlement_running:
                        self._draw_baccarat_chip(
                            tx, ty, bonus_display,
                            tags=('hand_wager', f'hand_wager_{idx}', 'main_side_wager'),
                            radius=17)
                    else:
                        self._draw_percentage_chip(
                            tx, ty, self.main_side_rate,
                            tags=('hand_wager', f'hand_wager_{idx}', 'main_side_wager'),
                            radius=17)
        ...
        for idx,hand in enumerate(self.hands):
            ...
            for j,card in enumerate(hand['cards']):
                ...
                self._render_card(
                    ...
                    golden=j in hand.get('golden_indices', set()))
            self._draw_hand_total_badge(idx,left,right,active=active)
        self._draw_main_hand_wagers()
        self._draw_main_side_feature()
        for tag in ('dealt_card','hand_wager','hand_label'):
            self.canvas.tag_raise(tag)
        self._render_insurance_chip()

    def _render_card(self, card, x, y, tags=('dealt_card',), golden=False):
        image = self._face_photo(card, golden=golden)
        if image is not None:
    ...
    def new_hand(self, cards=None, base_bet=0.0, contains_original=False,
                 ...
                 main_side_count=0, golden_indices=None):
        return {
            'cards': list(cards or []),
            'base_bet': float(base_bet),
            'double_added': 0.0,
            'contains_original': bool(contains_original),
            # System-awarded MAIN-SIDE chips are never part of hand_stake().
            # They remain bound to the Original Bet through splits and Doubles.
            'main_side_count': int(main_side_count),
            'main_side_return': 0.0,
            'main_side_settled': False,
            'golden_indices': set(golden_indices or ()),
    ...
    def deal_cards(self):
        ...
        fee = self.current_bet * self.MAIN_FEE_RATE
        if self.balance + 1e-9 < fee:
            messagebox.showwarning(
                '余额不足',
                f'开牌还需支付主注10%手续费 {self.format_money(fee)}；边注不收手续费。',
                parent=self.winfo_toplevel())
            return
        ...
        def after_old_cards():
            self.main_fee_collecting = True
            self.update_bet_chips()
            x0, y0, x1, _y1 = self.bet_spots['MAIN']['bounds']
            self._animate_settlement_chips(
                [(x1 - 18.0, y0 + 20.0, self.round_main_fee, 18)],
                ((self.GAME_X0 + self.GAME_X1) / 2.0, 105.0),
                280, 'main_fee_fly', after_fee)
    ...
    def _begin_round_after_bet_layout(self):
        ...
        # Keep the fee suppressed throughout the layout animation; resetting
        # this flag is safe only after round_active itself hides pre-deal fees.
        self.main_fee_collecting=False
        ...
        self.dealer_phase_active=False
        self.flash_mode=None
        self.main_side_rank=random.choice(BlackjackEngine.RANKS)
        self.main_side_rate=random.choices(
            [rate for rate, _prob in self.MAIN_SIDE_RATES],
            weights=[prob for _rate, prob in self.MAIN_SIDE_RATES], k=1)[0]
        self.main_side_match_count=0
        self.main_side_resolved=False
        self.main_side_roll_running=True
        self.main_side_feature_visible=False
        ...
        def deal_step(index=0):
            if index>=len(sequence):
                self.render_cards(); self._after_initial_deal(); return
            who=sequence[index]
            if who=='P':
        ...
        self._animate_main_side_roll(lambda:deal_step(0))

    def _after_initial_deal(self):
        hand=self.hands[0]
        self.main_side_match_count=sum(
            1 for _suit, rank in hand.get('cards', ()) if rank == self.main_side_rank)
        self.main_side_resolved=True
        self.render_cards()
        def close_gap():
            self.main_side_feature_visible=False
            hand['main_side_count']=self.main_side_match_count
            self.render_cards(); self.update_display()
            self._animate_reward_gap_closed(self._continue_after_initial_deal)
        if self.main_side_match_count > 0:
            self._animate_reward_chips_to_main(self.main_side_match_count, close_gap)
        else:
            close_gap()

    def _continue_after_initial_deal(self):
        self.animation_running=False
        hand=self.hands[0]
    ...
    def _deal_player_card_animated(self, hand_index, on_complete=None):
        if not (0 <= hand_index < len(self.hands)):
            ...
            return
        hand=self.hands[hand_index]
        card=self.draw_round_card(); card_index=len(hand['cards'])
        golden = bool(
            not self.main_side_resolved and hand_index == 0 and card_index < 2
            and card[1] == self.main_side_rank)
        if golden:
            hand.setdefault('golden_indices', set()).add(card_index)
        ...
        def done():
            ...
            if callable(on_complete): on_complete()
        self._animate_card_from_shoe(
            card, tx, ty, on_complete=done, flip=True,
            tag='round_animation_card', golden=golden)
    ...
    def split(self):
        ...
        golden_indices = set(hand.get('golden_indices', set()))
        ...
        left = self.new_hand(
            ...
            main_side_count=hand.get('main_side_count', 0),
            golden_indices={0} if 0 in golden_indices else set())
        right = self.new_hand(
            cards=[second], base_bet=hand['base_bet'],
            contains_original=False, from_split=True,
            split_aces=is_aces, needs_split_draw=True, main_side_count=0,
            golden_indices={0} if 1 in golden_indices else set())
    ...
    def _calculate_last_win_amount(self):
        ...
        returned_total += sum(
            max(0.0, float(hand.get('main_side_return', 0.0)))
            for hand in self.hands
        )
        return returned_total

    def _settle_main_side_rewards(self):
        """Credit awarded percentage chips only when their Original Bet wins."""
        for hand in self.hands:
            if hand.get('main_side_settled'):
                continue
            hand['main_side_settled'] = True
            hand['main_side_return'] = 0.0
            count = int(hand.get('main_side_count', 0))
            if count <= 0 or hand.get('base_settlement') != 'win':
                continue
            natural = bool(
                not hand.get('from_split', False)
                and BlackjackEngine.is_blackjack(hand.get('cards', ())))
            multiplier = 2.5 if natural else 2.0
            credit = (count * self.main_side_rate * self.round_original_bet
                      * multiplier)
            hand['main_side_return'] = credit
            self.balance += credit
            self.last_result_lines.append(
                f'奖励筹码：{count}枚 × {int(round(self.main_side_rate * 100))}%'
                f' × {multiplier:g} = {self.format_money(credit)}')
    ...
    def _finish_round(self, dealer_bj=False, phase_text=None):
        ...
        self._settle_main_side_rewards()
    ...
    def _reset_after_round(self):
        ...
        self.round_active=False; self.round_original_bet=0.0; self.round_main_fee=0.0
        self.main_fee_collecting=False
        self.main_side_rank=None; self.main_side_rate=0.0
        self.main_side_match_count=0; self.main_side_resolved=False
        self.main_side_roll_running=False; self.main_side_feature_visible=False
        self.pending_fee_animation=0.0
        self.round_side_bets={key:0.0 for key in self.SIDE_BET_KEYS}
    ...
    def update_display(self):
        ...
        if self.show_last_win:
            ...
            pending=(self.current_bet * (1.0 + self.MAIN_FEE_RATE)
    ...
    def show_game_instructions(self):
        ...
        tk.Label(
            header, text='投注叠堆黑杰克  BET STACKER BLACKJACK',
        ...
        add_rule(
            ...
            '加倍只追加与当前手原注相同金额，只再拿一张牌后自动停牌；奖励筹码不加倍。')
        add_rule(
            left_rules, '分A规则',
        ...
        add_rule(
            right_rules, '⑤ 主注手续费',
            '每次放置主注时，10%手续费筹码会从同一筹码架位置同步飞往手续费位；'
            '开牌前仅按Original Bet收取一次。六个边注、保险、分牌与加倍均不另收手续费。',
            accent='#7cc8aa')
        add_rule(
            right_rules, '⑥ 即时兑现',
        ...
        draw_action_tile(10, 128, '加倍  DOUBLE',
                         '只追加原始注码；奖励筹码不变，拿一张后停牌。', ORANGE, 'double')
        ...
        bet_card = make_card(
            '03  下注区域与限红',
            '主注 MAX $500K · 每个边注 MAX $100K · 主注手续费10%')
        bet_cv = tk.Canvas(
        ...
        tk.Label(
            bet_card,
            text='主注最低 $100、最高 $500,000；六个边注各自最高 $100,000。'
                 '开牌时只对主注收取10%手续费，边注不收手续费；右键可清除对应下注。',
            font=(font_name, FONT_SMALL), fg=MUTED, bg=PANEL,
        ...
        main_table = table_block(
            payout_card, '主注 / 保险', [
                ...
                ('奖励筹码：Original Bet普通获胜', '百分比 × Original Bet × 2'),
                ('奖励筹码：天然Blackjack获胜', '百分比 × Original Bet × 2.5'),
                ('奖励筹码：Push / 输 / 兑现', '收回'),
        ...
        add_rule(
            ...
            '才激活下一手并补该手的第一张分牌牌。')
        add_rule(
            notes_card, '奖励筹码抽取与命中',
            '每局系统随机A–K点数及25%/50%/75%/100%筹码'
            '抽奖直接显示在主注和明牌运气之间，点数牌只显示A–K'
            '仅检查玩家原始首两张：每有一张同点数便获得一枚，'
            '最多两枚；命中牌改用金色牌面。,'
            '首两张发完后，中奖筹码同时飞往原始下注，未中奖则直接收起；')
        add_rule(
            notes_card, '奖励筹码跟随规则',
            '奖励筹码绑定原始下注：分牌时随原始下注所在手移动；加倍不复制。'
            '只有该原始下注获胜才支付，其他情况均收回；兑现估值也不计入奖励筹码。')
        add_rule(
            notes_card, '即时兑现模型',
