# Blackjack Double Deck — 2-deck shoe / one split / hard 9-11 double / four side bets
import json
import math
import os
import random
...
BLACKJACK_VERSION = "DOUBLE_DECK_V2"
...
# -----------------------------------------------------------------------------
# 2-deck ENHC Blackjack shoe / hand helpers
# -----------------------------------------------------------------------------
class BlackjackEngine:
    ...
    def __init__(self, decks=2):
        ...
        self.cut_threshold = random.SystemRandom().randint(40, 50)
...
class BubbleBlackjackGame(tk.Frame):
    ...
    MAX_SPLIT_ACTIONS = 1       # one split action only -> at most 2 player hands
    MAX_HANDS = 2
    BLACKJACK_PROFIT = 1.5      # 3:2
    INSURANCE_PROFIT = 2.0      # 2:1
    SPLIT_ACES_ONE_CARD_ONLY = True
    SIDE_BET_KEYS = ('Perfect Pair', '21+3', 'Lucky Queens', 'Bust!')
    SIDE_BET_LABELS = {
        'Perfect Pair': '完美对子',
        '21+3': '21+3',
        'Lucky Queens': '幸运女王',
    ...
    SIDE_BET_ODDS_TEXT = {
        'Perfect Pair': '40/12/6:1',
        '21+3': '40/30/12/5:1',
        'Lucky Queens': '80/2:1',
        'Bust!': '250/100/50/10/2/1:1',
    }

    M_CHIP_COLOR = '#102d47'

    # phase_text is intentionally restricted to these five messages only.
    PHASE_BETTING = '双副牌黑杰克 · 请下注'
    PHASE_INSURANCE = '双副牌黑杰克 · 保险？'
    PHASE_EVEN_MONEY = '双副牌黑杰克 · 立刻获胜？'
    PHASE_PLAYING = '双副牌黑杰克 · 游戏中'
    PHASE_SETTLING = '双副牌黑杰克 · 结算中'
    ...
    def __init__(self, parent, balance=10000, user=None, on_back=None,
        ...
        # Double Deck Blackjack may be embedded by casino_games.py in the SAME Tk root.
    ...
    def draw_board(self):
        ...
        home = {
            ...
            # Four side bets are pulled inward toward MAIN for a denser, balanced layout.
            'Perfect Pair': ((250.0, 486.0, 410.0, 568.0), (330.0, 507.0), (330.0, 542.0)),
            '21+3': ((365.0, 536.0, 475.0, 614.0), (420.0, 554.0), (420.0, 586.0)),
            'Lucky Queens': ((675.0, 536.0, 785.0, 614.0), (730.0, 554.0), (730.0, 586.0)),
        ...
        round_layout = {
            ...
            # In-round side bets also stay closer to the centre instead of hugging the edges.
            'Perfect Pair': ((118.0, 504.0, 254.0, 586.0), (186.0, 525.0), (186.0, 560.0)),
            '21+3': ((284.0, 504.0, 394.0, 586.0), (339.0, 525.0), (339.0, 560.0)),
            'Lucky Queens': ((756.0, 504.0, 866.0, 586.0), (811.0, 525.0), (811.0, 560.0)),
            'Bust!': ((896.0, 504.0, 1052.0, 586.0), (974.0, 525.0), (974.0, 560.0)),
        }
    ...
    def draw_bottom_controls(self):
        ...
        specs = [
            ('hit', 270, 638, 474, 720, '要牌', self.hit, '#315b72'),
            ('stand', 484, 638, 688, 720, '停牌', self.stand, '#5b4938'),
            ('double', 698, 638, 902, 720, '加倍', self.double, '#a56d2c'),
            ('split', 912, 638, 1138, 720, '分牌', self.split, '#70439a'),
        ]
    ...
    def _draw_side_bet_progress_badges(self):
        ...
        dealer-dependent result is settled.
        """
    ...
    def load_runtime_store(self):
        try:
            ...
            if int(data.get('version', 0)) != 3 or not isinstance(deck, list) or len(deck) != self.engine.decks * 52:
                return False
            if not (40 <= threshold <= 50) or not (0 <= index <= len(deck)):
                return False
    ...
    def _show_cut_dialog(self, second=False):
        ...
        cv.create_text(380, 76, text='确认后会将两叠牌交换位置，然后直接进入下注',
        ...
        valid_left = shoe_x0 + deck_width * 12 / 104.0
        valid_right = shoe_x0 + deck_width * 92 / 104.0
        ...
        selected = {'value': rng.randint(12, 92)}
        animating = {'value': False}
        result = [None]
        def value_to_x(value): return shoe_x0 + deck_width * value / 104.0
        def x_to_value(xx):
            value = int(round((xx - shoe_x0) / float(deck_width) * 104))
        ...
        def finish_cut_animation():
            ...
            cv.create_text(380,292,text='切牌完成 · 准备下注',font=('微软雅黑',11,'bold'),
        ...
        def begin_cut_animation(value=None):
            ...
            if value is not None:
                selected['value']=max(12,min(92,int(value))); draw_cut_card()
        ...
        def random_confirm(_event=None):
            if animating['value']: return 'break'
            selected['value']=rng.randint(12,92); draw_cut_card(); dialog.after(100,begin_cut_animation,selected['value']); return 'break'
    ...
    def _draw_main_hand_wagers(self):
        ...
        for idx, hand in enumerate(self.hands):
            ...
            show_double = dbl > 0.0
            if self.settlement_running:
                # Only genuinely losing components disappear. Push/refund
    ...
    def _player_hand_badge_fill(self, hand_index):
        ...
        At settlement the final hand result takes precedence: win=green,
    ...
    def can_double(self, hand):
        if not (hand and hand['status'] == 'active' and len(hand['cards']) == 2
                and not hand.get('split_aces', False)
                and self.balance + 1e-9 >= hand['base_bet']):
            return False
        total, soft = BlackjackEngine.hand_value(hand['cards'])
        # Double Deck rule: double is allowed only on a HARD 9, 10 or 11.
        # The same test applies after a split, so DAS remains available only
        # when the newly completed two-card split hand is hard 9-11.
        return (not soft) and total in (9, 10, 11)
    ...
    def hit(self):
        ...
        self.animation_running=True
        # V10: visually and functionally lock ALL four action buttons before
    ...
    def _settle_initial_side_bets(self):
        ...
        # Perfect Pair (double-deck repricing): perfect > coloured > mixed.
        odds = None
        label = '完美对子'
        if p0[1] == p1[1]:
            if p0[0] == p1[0]:
        ...
        # 21+3: straight flush > trips > straight > flush.
        if suits_same and straight:
            result = (40, '同花顺')
        elif ranks_same:
            result = (30, '三条')
        elif straight:
        ...
        # Lucky Queens: the player's first two cards only.
        # Priority: suited Q+K, then any other two cards of the same suit.
        same_suit = p0[0] == p1[0]
        ranks = {p0[1], p1[1]}
        if same_suit and ranks == {'Q', 'K'}:
            result = (80, '同花Q+K')
        elif same_suit:
            result = (2, '同花')
        else:
            result = (None, '未中')
        self._credit_side_bet('Lucky Queens', *result)
    ...
    def _start_dealer_turn(self):
        ...
        if not bust_wager:
            if all_dead and self.insurance_bet <= 0.0:
                # All hands are already lost: no hole card is needed.
                self.animation_running = True
        ...
        def after_second():
            dealer_bj = BlackjackEngine.is_blackjack(self.dealer_cards[:2])
            if not bust_wager:
                # Insurance with every player hand already bust needs
    ...
    def _calculate_last_win_amount(self):
        ...
        PUSH, OBO/refunds, Even Money, side-bet credits and insurance
    ...
    def _finish_round(self, dealer_bj=False, phase_text=None):
        ...
        # hand is a true win/refund, white keeps precedence for the whole MAIN.
    ...
    def show_game_instructions(self):
        ...
        tk.Label(
            header, text='双副牌黑杰克  BLACKJACK DOUBLE DECK',
        ...
        add_rule(
            left_rules, '分牌 / 加倍',
            '同点数价值的两张牌最多分牌1次（共2手）；允许分牌后加倍（DAS）。'
            '加倍仅限硬9、硬10、硬11，需追加与当前手原注相同金额，只再拿一张牌后自动停牌。')
        add_rule(
            left_rules, '分A规则',
            'A-A可分牌。由于整局最多只允许分牌1次，分A后每手只补一张并自动停牌，不可再次分A。')
        add_rule(
            left_rules, '庄家A与保险',
        ...
        add_rule(
            right_rules, '③ 自动换新牌靴',
            '每个新牌靴随机设定40–50张的洗牌阈值；剩余牌数达到该阈值时，在下一局开始前进入新的洗牌与切牌流程。'
            '切牌完成后直接进入下注，不再额外弃牌。',
            accent='#7cc8aa')
        ...
        draw_action_tile(10, 128, '加倍  DOUBLE',
                         '仅硬9–11可追加同额下注；只拿一张牌后自动停牌。', ORANGE, 'double')
        draw_action_tile(470, 128, '分牌  SPLIT',
                         '同点值两张牌最多分牌1次（共2手）；DAS仅限硬9–11。', PURPLE, 'split')
        draw_action_tile(245, 246, '庄家明牌A的决策',
        ...
        tk.Label(
            bet_card,
            text='主注最低 $100、最高 $500,000；四个边注各自最高 $100,000。右键可清除对应下注。',
            font=(font_name, FONT_SMALL), fg=MUTED, bg=PANEL,
        ...
        # 05 边注赔付
        # ================================================================
        side_card = make_card('05  边注赔付表', '四个边注 · 固定赔付 · 按利润赔率结算')
    ...
    def exit_game(self):
        ...
        # Refund only unresolved escrow on a forced exit. Even Money is already settled
        # and is not refunded again.
        if not self.round_active:
...
def main(parent=None, balance=10000, user=None, on_back=None,
    ...
    root = tk.Tk()
    root.title('Blackjack Double Deck')
