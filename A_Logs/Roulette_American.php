            ...
            <div class="data-panel">
                ...
                <div class="panel-right">
                    ...
                    <div class="sector-table">
                        ...
                        <div class="sector-row"><span class="sector-label">00</span><span class="sector-value" id="r-sector-00">0%</span></div>
                    </div>
                </div>
            </div>
        </div>
...
<script>
    (function() {
        const DATA_URL = './Json/Roulette_American.json';
        let record2Data = [];
        ...
        // 计算分区计数 (1-12, 13-24, 25-36, 0, 00)
        function computeSectorCounts(records) {
            let s1=0, s2=0, s3=0, s0=0, s00=0;
            for (const rec of records) {
                const result = rec.result; // 字符串，如 "0", "00", "5"
                if (result === '00') {
                    s00++;
                } else {
        ...
        function renderSectorStats(sectorCounts, total) {
            if (total === 0) {
                ...
                document.getElementById('r-sector-00').textContent = '0%';
                return;
            }
            ...
            document.getElementById('r-sector-00').textContent = pct(sectorCounts.s00);
        }
        ...
        function renderBarChart(counts) {
            ...
            // 构造数字列表：0, 00, 1-36
            const numbers = [];
            numbers.push({ num: '0', count: counts['0'] || 0 });
            numbers.push({ num: '00', count: counts['00'] || 0 });
            for (let i = 1; i <= 36; i++) {
                const key = String(i);
