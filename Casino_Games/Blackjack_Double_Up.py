...
class BubbleBlackjackGame(tk.Frame):
    ...
    MAX_SPLIT_ACTIONS = 3
    MAX_HANDS = 4
    BLACKJACK_PROFIT = 1.5
    INSURANCE_PROFIT = 2.0
    SPLIT_ACES_ONE_CARD_ONLY = True

    SIDE_BET_KEYS = ('Perfect Pair', '21+3', 'Sixteen', 'Crazy 7', 'Hot 21', 'Bust!')
    SIDE_BET_LABELS = {
        'Perfect Pair': '完美对子',
        '21+3': '21+3',
        'Sixteen': '16点',
    ...
    SIDE_BET_ODDS_TEXT = {
        ...
        'Sixteen': '3/5/10/50/100:1',   # 对应2,3,4,5,6张
    ...
    PHASE_BETTING = '无牌加倍黑杰克 · 请下注'
    PHASE_INSURANCE = '无牌加倍黑杰克 · 保险？'
    PHASE_EVEN_MONEY = '无牌加倍黑杰克 · 立刻获胜？'
    PHASE_PLAYING = '无牌加倍黑杰克 · 游戏中'
    PHASE_SETTLING = '无牌加倍黑杰克 · 结算中'
    ...
    def __init__(self, parent, balance=10000, user=None, on_back=None,
        ...
        self.dealer_16_main_push = False
    ...
    def draw_animation_panel(self):
        ...
        self.rule_strip_text = c.create_text(
            (rx0 + rx1) / 2, (ry0 + ry1) / 2,
            text='除庄家软17，任何16点或以上停牌   保险2:1   黑杰克3:2   玩家任何21点必胜',
    ...
    def draw_board(self):
        ...
        home = {
            ...
            'Sixteen': ((458.0, 552.0, 568.0, 618.0), (513.0, 567.0), (513.0, 594.0)),
        ...
        round_layout = {
            ...
            'Sixteen': ((284.0, 504.0, 414.0, 586.0), (349.0, 525.0), (349.0, 560.0)),
    ...
    def draw_bottom_controls(self):
        ...
        specs = [
            ...
            ('double_no_card', 958, 638, 1138, 720, '无牌加倍', self.double_no_card, '#7c3b40'),
        ]
    ...
    def _settlement_losing_chip_sources(self):
        ...
        for idx,hand in enumerate(self.hands):
            ...
            if float(hand.get('double_no_card_added',0.0))>0 and hand.get('double_no_card_settlement')=='lose':
                dbl_pos = layout[idx]['double'] if layout[idx].get('double') else layout[idx]['base']
                x = dbl_pos[0] + 15.0
                y = dbl_pos[1]
                sources.append((x,y,float(hand['double_no_card_added']),20))
    ...
    def _settlement_return_chip_sources(self):
        ...
        for idx,hand in enumerate(self.hands):
            ...
            amount=self._component_return_amount(hand,'double_no_card')
            if amount>0.0:
                dbl_pos = layout[idx]['double'] if layout[idx].get('double') else layout[idx]['base']
                x = dbl_pos[0] + 15.0
                y = dbl_pos[1]
    ...
    def _component_return_amount(self, hand, component):
        ...
        elif component == 'double':
            amount = float(hand.get('double_added', 0.0))
            state = hand.get('double_settlement')
        elif component == 'double_no_card':
            amount = float(hand.get('double_no_card_added', 0.0))
            state = hand.get('double_no_card_settlement')
        else:
            return 0.0
        ...
        if state == 'win':
            if component == 'base':
                natural = (hand.get('status') != 'even_money'
    ...
    def _draw_main_hand_wagers(self):
        ...
        for idx, hand in enumerate(self.hands):
            ...
            dbl_nc = float(hand.get('double_no_card_added', 0.0))
            base_state = hand.get('base_settlement')
            double_state = hand.get('double_settlement')
            double_nc_state = hand.get('double_no_card_settlement')
            show_base = base > 0.0
            show_double = dbl > 0.0
            show_double_nc = dbl_nc > 0.0
            if self.settlement_running:
                show_base = show_base and base_state != 'lose'
                show_double = show_double and double_state != 'lose'
                show_double_nc = show_double_nc and double_nc_state != 'lose'

            self.main_wager_positions[idx] = {
                'base': pos['base'],
                'double': pos['double'] if dbl > 0.0 else None,
                'double_no_card': (pos['double'][0] + 15.0, pos['double'][1]) if dbl_nc > 0.0 else None,
                'show_base': show_base,
                'show_double': show_double,
                'show_double_nc': show_double_nc,
            }

            base_display = base
            double_display = dbl
            double_nc_display = dbl_nc
            if self.settlement_running and self.flash_mode == 'win':
                ...
                double_nc_display = self._component_return_amount(hand, 'double_no_card')
            ...
            if show_double_nc and double_nc_display > 0.0:
                x = pos['double'][0] + 15.0
                y = pos['double'][1]
                self._draw_baccarat_chip(
                    x, y, double_nc_display,
                    tags=('hand_wager', f'hand_wager_{idx}', 'double_no_card_wager'), radius=20)
    ...
    def _dealer_16_push_marker_target(self):
        """主注区右上角放置 placard 的位置 (top-left)"""
        spot = self.bet_spots.get('MAIN')
        if spot:
            _x0, y0, x1, _y1 = spot['bounds']
        else:
            _x0, y0, x1, _y1 = (455.0, 452.0, 695.0, 598.0)
        # 放置在主注 oval 的右上角外侧，不遮挡筹码
        return float(x1) - 18.0, float(y0) - 54.0

    def _dealer_16_push_marker_source(self):
        """指示牌飞入的起点（庄家区域左上角）"""
        return float(self.GAME_X0) + 28.0, 38.0

    def _draw_dealer_16_push_marker_at(self, x, y):
        """绘制 50×70 的白色 '16点 / 平局' 指示牌"""
        self.canvas.delete('dealer_16_push_marker')
        w, h = 50.0, 70.0
        rect = self.canvas.create_rectangle(
            x, y, x + w, y + h,
            fill='#ffffff', outline='#111111', width=2,
            tags=('dealer_16_push_marker', 'settlement_flash'))
        label = self.canvas.create_text(
            x + w / 2.0, y + h / 2.0,
            text='16点\n平局', justify='center',
            font=(self.cn_font, 12, 'bold'), fill='#111111',
            tags=('dealer_16_push_marker', 'settlement_flash'))
        self.canvas.tag_raise('dealer_16_push_marker')
        return rect, label

    def _animate_dealer_16_push_marker(self, to_table, on_complete=None):
        """飞入/飞出指示牌，用时 0.20 秒"""
        if to_table and not self.dealer_16_main_push:
            if callable(on_complete):
                on_complete()
            return
        source = self._dealer_16_push_marker_source()
        target = self._dealer_16_push_marker_target()
        start = source if to_table else target
        end = target if to_table else source
        if not to_table and not self.canvas.find_withtag('dealer_16_push_marker'):
            if callable(on_complete):
                on_complete()
            return

        rect, label = self._draw_dealer_16_push_marker_at(*start)
        w, h = 50.0, 70.0
    ...
    def new_hand(self, cards=None, base_bet=0.0, contains_original=False,
        ...
        return {
            ...
            'double_no_card_settlement': None,
            'settlement_return': 0.0,
        }

    @staticmethod
    def hand_stake(hand):
        return (float(hand.get('base_bet', 0.0)) +
                float(hand.get('double_added', 0.0)) +
                float(hand.get('double_no_card_added', 0.0)))
    ...
    def can_double_no_card(self, hand):
        if not hand or hand['status'] != 'active' or len(hand['cards']) != 2:
            return False
        if BlackjackEngine.is_blackjack(hand['cards']):
            return False
        if self.balance + 1e-9 < hand['base_bet']:
            return False
        return True
    ...
    # 无牌加倍：延迟0.5秒再推进
    def double_no_card(self):
        hand = self.active_hand()
        if not self.can_double_no_card(hand) or self.animation_running:
            return
        ...
        add = hand['base_bet']
        self.balance -= add
        hand['double_no_card_added'] += add
        hand['status'] = 'stood'          # 不补牌，直接停牌
        hand['result'] = '无牌加倍'
        self.save_balance()
        self.render_cards()
        self.update_display()

        # 延迟 0.5 秒后推进到下一手或庄家回合
        def delayed_advance():
            self.animation_running = False   # 允许庄家回合启动
            self._advance_hand()

        self.after(500, delayed_advance)
    ...
    # 16点边注：庄家最终为16点（无论软硬）时按张数赔付
    def _settle_sixteen_points(self):
        key = 'Sixteen'
        if key in self.side_bet_results:
            return
        dealer_cards = self.dealer_cards
        if not dealer_cards:
            self._credit_side_bet(key, None, '庄家无牌')
            return
        total, soft = BlackjackEngine.hand_value(dealer_cards)
        if total == 16:
            count = len(dealer_cards)
            # 6张及以上统一100:1
            if count >= 6:
                odds = 100
                label = f'庄{count}张16点'
            else:
                odds_map = {2: 3, 3: 5, 4: 10, 5: 50}
                odds = odds_map.get(count, None)
                label = f'庄{count}张16点' if odds is not None else '庄家16点张数超出'
            if odds is not None:
                self._credit_side_bet(key, odds, label)
            else:
                self._credit_side_bet(key, None, '庄家16点张数超出')
        else:
            self._credit_side_bet(key, None, '庄家非16点')
    ...
    def _resolve_finished_hands_after_insurance_probe(self):
        ...
        for idx, hand in enumerate(self.hands):
            ...
            if hand.get('status') == 'bust' or total > 21:
                ...
                hand['double_no_card_settlement'] = 'lose' if hand.get('double_no_card_added', 0.0) > 0 else None
    ...
    def _dealer_draw_until_done(self):
        """庄家补牌，停牌条件：16-21停牌（软17除外）"""
        total, soft = BlackjackEngine.hand_value(self.dealer_cards)
        # 爆牌或满足停牌条件（16-21点，且不是软17）
        if total > 21 or ((total >= 16 and total <= 21) and not (total == 17 and soft)):
    ...
    def _resolve_dealer_blackjack(self):
        ...
        self._maybe_settle_crazy7()
        self._settle_bust_side_bet()
        self._settle_sixteen_points()   # 庄家BJ不是16点，输
        lines = ['庄家 Blackjack']
    ...
    def _resolve_normal_round(self):
        self._maybe_settle_crazy7()
        self._settle_bust_side_bet()
        self._settle_sixteen_points()
        dealer_total, dealer_soft = BlackjackEngine.hand_value(self.dealer_cards)
        dealer_bust = dealer_total > 21
        dealer_sixteen = (not dealer_bust and dealer_total == 16)
        ...
        for idx, hand in enumerate(self.hands):
            ...
            # ----- 庄家16点特殊处理 -----
            if dealer_sixteen:
                # 玩家21点：主注和加倍按1:1赢，无牌加倍输
                if player_total == 21:
                    base_win = hand['base_bet'] * 2.0
                    double_win = hand.get('double_added', 0.0) * 2.0 if hand.get('double_added', 0.0) > 0 else 0.0
                    self.balance += base_win + double_win
                    hand['settlement'] = 'win'
                    hand['base_settlement'] = 'win'
                    hand['double_settlement'] = 'win' if hand.get('double_added', 0.0) > 0 else None
                    hand['double_no_card_settlement'] = 'lose' if hand.get('double_no_card_added', 0.0) > 0 else None
                    hand['settlement_return'] = base_win + double_win
                    if natural:
                        hand['result'] = f'Blackjack 1:1 (庄16) +{self.format_money(hand["base_bet"])}'
                    else:
                        hand['result'] = f'21点 1:1 (庄16) +{self.format_money(hand["base_bet"])}'
                    lines.append(f'手{idx+1}：21点 1:1 (庄16)')
                else:
                    # 非21点：主注和加倍平局，无牌加倍输
                    base_push = hand['base_bet']
                    double_push = hand.get('double_added', 0.0)
                    self.balance += base_push + double_push
                    hand['settlement'] = 'push'
                    hand['base_settlement'] = 'push'
                    hand['double_settlement'] = 'push' if double_push > 0 else None
                    hand['double_no_card_settlement'] = 'lose' if hand.get('double_no_card_added', 0.0) > 0 else None
                    hand['settlement_return'] = base_push + double_push
                    hand['result'] = '庄16停牌，主注Push，无牌加倍输'
                    lines.append(f'手{idx+1}：庄16停牌，主注Push，无牌加倍输')

                if not self.dealer_16_main_push:
                    self.dealer_16_main_push = True
                    self._animate_dealer_16_push_marker(True)
                continue  # 跳过后续常规结算
            ...
            if natural:
                ...
                hand['base_settlement'] = 'win'
                hand['double_settlement'] = 'win' if hand.get('double_added', 0.0) > 0 else None
                hand['double_no_card_settlement'] = 'win' if hand.get('double_no_card_added', 0.0) > 0 else None
                ...
                hand['double_no_card_settlement'] = 'push' if hand.get('double_no_card_added', 0.0) > 0 else None
                hand['settlement_return'] = stake
    ...
    def _reset_after_round(self):
        ...
        def after_layout():
            if self.dealer_16_main_push:
                self._animate_dealer_16_push_marker(False)
                self.dealer_16_main_push = False
    ...
    def update_display(self):
        ...
        self._set_button(self.action_buttons.get('double_no_card'),
                         active and self.can_double_no_card(hand), '无牌加倍')
    ...
    def show_game_instructions(self):
        ...
        tk.Label(
            header, text='无牌加倍黑杰克  DOUBLE UP BLACKJACK',
            font=(font_name, FONT_TITLE, 'bold'), fg=GOLD, bg=HEADER
        ).place(x=24, y=10)

        tk.Label(
            header, text='玩法 · 切牌 · 决策按钮 · 下注区域 · 限红 · 赔付表',
        ...
        def make_card(title, subtitle=None, pady=(8, 8)):
            ...
            tk.Label(
                ...
                fg=GOLD, bg=PANEL, anchor='w'
            ).pack(side=tk.LEFT)
            if subtitle:
                tk.Label(
                    head, text=subtitle, font=(font_name, FONT_SMALL),
                    fg=MUTED, bg=PANEL, anchor='e'
                ).pack(side=tk.RIGHT)
            return card
        ...
        add_rule(
            left_rules, '无牌加倍',
            '玩家可在两张牌后（黑杰克除外）选择“无牌加倍”，追加一倍赌注但不拿第三张牌。'
            '结果与主注相同（赢/平/输），但庄家16点停牌时无牌加倍直接输，且不适用OBO。'
        )
        ...
        add_rule(
            right_rules, '⑤ 庄家16点特殊处理',
            '庄家最终为16点时（无论软硬），所有主注和加倍均Push，无牌加倍输。'
            '若玩家为自然黑杰克或爆牌，则按正常规则结算。',
            accent='#7cc8aa'
        )
        ...
        button_cv = tk.Canvas(
            button_card, width=930, height=370,
        ...
        def draw_small_card(cv, x, y, rank='A', suit='♠', red=False):
            cv.create_rectangle(
                x, y, x + 40, y + 55,
                fill='#f5f0e5', outline='#d7cbb9', width=2
            )
            cv.create_text(
                x + 11, y + 13, text=rank,
        ...
        def action_icon(cv, kind, x, y, color):
            if kind == 'hit':
                ...
                cv.create_text(x + 63, y + 42, text='+', font=('Arial', FONT_LARGE, 'bold'), fill=BLACK)
                ...
                cv.create_oval(x + 5, y + 15, x + 50, y + 60, fill=color, outline='#f3df9a', width=2)
                cv.create_oval(x + 34, y + 3, x + 79, y + 48, fill=color, outline='#f3df9a', width=2)
                cv.create_text(x + 42, y + 32, text='2×', font=('Arial', FONT_SMALL, 'bold'), fill=WHITE)

            elif kind == 'split':
                draw_small_card(cv, x + 2, y + 3, '8', '♠')
                draw_small_card(cv, x + 44, y + 3, '8', '♦', True)

            elif kind == 'double_no_card':
                cv.create_oval(x + 5, y + 15, x + 50, y + 60, fill=color, outline='#f3df9a', width=2)
                cv.create_oval(x + 34, y + 3, x + 79, y + 48, fill=color, outline='#f3df9a', width=2)
                cv.create_text(x + 42, y + 32, text='无牌', font=('Arial', FONT_SMALL, 'bold'), fill=WHITE)
                ...
                cv.create_text(x + 69, y + 17, text='YES', font=('Arial', FONT_SMALL, 'bold'), fill=WHITE)
                cv.create_text(x + 69, y + 49, text='NO', font=('Arial', FONT_SMALL, 'bold'), fill=WHITE)

        def draw_action_tile(x, y, title, detail, color, kind):
            w, h = 440, 108
            button_cv.create_rectangle(x + 3, y + 4, x + w + 3, y + h + 4,
                                       fill='#090c0a', outline='')
            button_cv.create_rectangle(x, y, x + w, y + h,
        ...
        draw_action_tile(
            10, 128,
            '加倍  DOUBLE',
            '追加与该手原注相同金额的赌注，只拿一张牌后自动停牌。',
            ORANGE, 'double'
        )
        draw_action_tile(
            470, 128,
            '分牌  SPLIT',
            '同点数价值的两张牌可分牌，需追加与当前手原注相同的赌注。',
            PURPLE, 'split'
        )
        draw_action_tile(
            10, 246,
        ...
        bet_cv.create_text(30, 30, anchor='w', text='可下注区域示意',
                           font=(font_name, FONT_MEDIUM, 'bold'), fill=TEXT)
        bet_cv.create_text(900, 30, anchor='e', text='主注最低 $100',
        ...
        bet_cv.create_text(465, 195, text='MAX $500K',
                           font=(font_name, FONT_MEDIUM, 'bold'), fill=GOLD)

        side_spots = [
            (40, 74, 225, 140, '完美对子', '$100K'),
            (40, 172, 225, 238, '21+3', '$100K'),
            (240, 224, 405, 288, '16点', '$100K'),
            (525, 224, 690, 288, '疯狂7', '$100K'),
            (705, 172, 890, 238, '热辣21', '$100K'),
            (705, 74, 890, 140, '爆牌!', '$100K'),
        ]

        for x0, y0, x1, y1, label, limit in side_spots:
            bet_cv.create_oval(x0, y0, x1, y1, fill=GREEN_2,
                               outline='#9de46f', width=3)
            bet_cv.create_text((x0 + x1) / 2, (y0 + y1) / 2 - 13,
                               text=label, font=(font_name, FONT_SMALL, 'bold'),
                               fill=WHITE)
            bet_cv.create_text((x0 + x1) / 2, (y0 + y1) / 2 + 14,
                               text=f'MAX {limit}', font=(font_name, FONT_SMALL, 'bold'),
                               fill=GOLD)
        ...
        tk.Label(
            ...
            text=(
                ...
                '边注： 完美对子、21+3、16点、疯狂7、热辣21、爆牌!'
            ),
            font=(font_name, FONT_SMALL),
            fg=MUTED, bg=PANEL, anchor='w', justify=tk.LEFT, wraplength=920
        ...
        main_table = table_block(
            ...
            [
                ...
                ('无牌加倍获胜', '1:1'),
                ('无牌加倍平局 / 输（庄家16点停牌时直接输）', '按主注结果，但庄16点直接输'),
                ('庄家16点：主注和加倍', 'Push退回'),
        ...
        add_rule(
            notes_card,
            '无牌加倍与庄家16点',
            '庄家最终为16点时，无牌加倍直接输（不退还），主注和加倍Push。\n'
            '此规则独立于OBO，适用于所有手牌。'
        )
        add_rule(
            notes_card,
            '分牌行动顺序',
            '分牌后先补第一手并完成行动，再激活下一手。'
        )
