...
def draw_roulette_static(canvas, scale=BOARD_SCALE):
    ...
    # 沿用原棋盘画布、配色与筹码交互，仅把下注内容换成幸运之轮七选项。
...
class RouletteGameGUI(tk.Frame):
    ...
    def _add_help_button_on_board(self):
        ...
        # 保留原帮助按钮的位置和大小。
        btn_x1, btn_y1 = 22.0, 200.5
        btn_x2, btn_y2 = 57.5, 250.0
    ...
    def _build_bet_spots(self):
        ...
        # 幸运之轮只有七种单项下注。继续使用原来的 spot/chip/click/flash 体系。
        for result, bounds in lucky_board_bounds().items():
            x1, y1, x2, y2 = bounds
            spots.append({
                "id": f"straight_{result}",
                "type": "straight",
                "label": result,
                "numbers": {result},
                "payout": lucky_net_odds(result),
                "kind": "cell",
                "bounds": bounds,
    ...
    def show_game_instructions(self):
        ...
        text = (
            ...
            "9. 历史数据保存在 A_Logs/Json/lucky_6.json。\n"
        )
        frm = tk.Frame(win)
        frm.pack(fill=tk.BOTH, expand=True, padx=16, pady=16)
