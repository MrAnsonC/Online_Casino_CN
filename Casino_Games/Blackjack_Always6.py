...
# 永6黑杰克 — fixed dealer 6 / 0.61:1 main wins / Lucky Ladies
import json
import math
import os
import random
...
BLACKJACK_VERSION = "Always6-CSM-V8"
...
class BubbleBlackjackGame(tk.Frame):
    ...
    MAX_HANDS = 4
    DEALER_FIXED_CARD = ('Always', '6')  # Virtual card: never drawn/returned to CSM.
    MAIN_PROFIT = 0.61         # 0.61:1
    BLACKJACK_PROFIT = 1.5      # 3:2
    INSURANCE_PROFIT = 2.0      # 2:1
    SPLIT_ACES_ONE_CARD_ONLY = True
    SIDE_BET_KEYS = ('Perfect Pair', 'Crazy 7', 'Lucky Ladies', 'Simple Bust')
    SIMPLE_BUST_FRACTIONS = (0.25, 0.50, 0.75, 1.0)
    SIDE_BET_LABELS = {
        'Perfect Pair': '完美对子',
        'Crazy 7': '疯狂7',
        'Lucky Ladies': '幸运女士',
    ...
    PHASE_BETTING = '永6黑杰克 · 请下注'
    PHASE_INSURANCE = '永6黑杰克 · 保险？'
    PHASE_EVEN_MONEY = '永6黑杰克 · 立刻获胜？'
    PHASE_PLAYING = '永6黑杰克 · 游戏中'
    PHASE_SETTLING = '永6黑杰克 · 结算中'
    ...
    def __init__(self, parent, balance=10000, user=None, on_back=None,
        ...
        import uuid
        self.csm_game_id = f"Always6Blackjack:{self.username or 'local'}:{uuid.uuid4().hex}"
        ...
        self.ace_decision_mode = None  # Legacy control-layout compatibility.
        self.simple_bust_offer_open = False
        self.bet_layout_mode = 'home'
        self.previous_round_cards_present = False
        self.hands = []
        self.active_hand_index = 0
        self.dealer_cards = [self.DEALER_FIXED_CARD]
        ...
        if directory:
            try:
                ...
                if self.external_card_images and self.external_back_image is not None:
                    ...
                    self._load_always6_asset(directory)
                    return
            except Exception:
    ...
    def _load_always6_asset(self, directory):
        """Prefer supplied Always6.png; otherwise draw a neutral fixed-six card."""
        candidates = [os.path.join(directory, 'Always6.png')] if directory else []
        candidates.append(os.path.join(_THIS_DIR, 'Always6.png'))
        base = None
        for path in candidates:
            try:
                with Image.open(path) as source:
                    base = source.convert('RGBA').resize(
                        (100, 140), getattr(Image, 'Resampling', Image).LANCZOS)
                break
            except (OSError, ValueError):
                continue
        if base is None:
            base = self._make_fallback_card_pil(self.DEALER_FIXED_CARD, True)
        self.external_card_pil[self.DEALER_FIXED_CARD] = base
        self.external_card_images[self.DEALER_FIXED_CARD] = ImageTk.PhotoImage(base, master=self)
    ...
    def _make_fallback_card_pil(self, card, face_up=True):
        ...
        if not face_up:
            ...
            return image
        suit, rank = card
        suit_map = {'Club': '♣', 'Diamond': '♦', 'Heart': '♥', 'Spade': '♠', 'Always': '固定'}
        ...
        if suit == 'Always':
            draw.text((9, 58), 'ALWAYS 6', fill=color)
        else:
    ...
    def create_ui(self):
        ...
        self._render_fixed_dealer_card()
    ...
    def draw_animation_panel(self):
        ...
        self.rule_strip_text = c.create_text(
            (rx0 + rx1) / 2, (ry0 + ry1) / 2,
            text='庄家固定6点 · 任何17点停牌   主注0.61:1   黑杰克3:2',
    ...
    def draw_board(self):
        ...
        # The five individual betting ovals keep their established dimensions.
        ...
        home = {
            ...
            'Crazy 7': ((495.0, 550.0, 655.0, 616.0), (575.0, 565.0), (575.0, 594.0)),
            'Lucky Ladies': ((770.0, 486.0, 930.0, 568.0), (850.0, 507.0), (850.0, 542.0)),
            'Simple Bust': ((954.0, 486.0, 1124.0, 568.0), (1039.0, 507.0), (1039.0, 542.0)),
        }
        round_layout = {
            'MAIN': ((455.0, 452.0, 695.0, 598.0), (575.0, 480.0), (575.0, 542.0)),
            'Perfect Pair': ((58.0, 504.0, 218.0, 586.0), (138.0, 525.0), (138.0, 560.0)),
            'Crazy 7': ((248.0, 504.0, 408.0, 586.0), (328.0, 525.0), (328.0, 560.0)),
            'Lucky Ladies': ((770.0, 504.0, 930.0, 586.0), (850.0, 525.0), (850.0, 560.0)),
            'Simple Bust': ((954.0, 504.0, 1124.0, 586.0), (1039.0, 525.0), (1039.0, 560.0)),
        }
    ...
    def _set_round_control_visibility(self, in_round):
        """Switch bottom strip between betting, post-deal wager, and hand actions."""
    ...
    def _animate_previous_round_cards_out(self, on_complete=None):
        ...
        if not self.canvas.find_withtag('dealt_card'):
            self._move_fixed_dealer_card(525, 30)
            self.previous_round_cards_present = False
            if callable(on_complete): on_complete()
            return
        ...
        fixed_x, fixed_y = self._fixed_dealer_card_position
        ...
        def frame(step=1):
            ...
            self._move_fixed_dealer_card(
                fixed_x + (525 - fixed_x) * e,
                fixed_y + (30 - fixed_y) * e)
            if step < steps: self._queue(frame_ms, frame, step+1)
            else:
    ...
    def _draw_side_bet_progress_badges(self):
        ...
        Crazy 7 updates its provisional tier as real cards 0, 1 and 2 arrive.
        """
    ...
    def place_side_bet(self, key):
        if (key == 'Simple Bust' or key not in self.SIDE_BET_KEYS or not self.accept_bets or self.round_active
    ...
    def repeat_last_bet(self):
        ...
        targets['Simple Bust'] = 0.0  # Must be chosen again after the opening cards.
    ...
    def _draw_single_temp_card(self, card, x, y, tag, face_up=True):
        ...
        if face_up and card is not None:
            suit, rank = card
            symbol = {'Club': '♣', 'Diamond': '♦', 'Heart': '♥', 'Spade': '♠', 'Always': '固定'}.get(suit, suit[:1])
    ...
    def _component_return_amount(self, hand, component):
        ...
        if amount <= 0.0 or not state:
            return 0.0
        if state == 'win':
            # Only an unsplit natural Blackjack receives 3:2. Split 21 is 0.61:1.
            ...
            return amount * (1.0 + self.BLACKJACK_PROFIT) if natural else amount * (1.0 + self.MAIN_PROFIT)
    ...
    def _dealer_card_target(self, card_index):
        ...
        # Normal centered dealer fan, including the virtual first card.
        overlap = 42
        total_w = 100 + max(0, count - 1) * overlap
        return 575 - total_w / 2 + card_index * overlap, 30

    def _render_fixed_dealer_card(self):
        """Persistent table card: excluded from dealt-card collection and redraws."""
        if not self.canvas.find_withtag('fixed_dealer_card'):
            self._render_card(self.DEALER_FIXED_CARD, 525, 30,
                              tags=('fixed_dealer_card',))
            self._fixed_dealer_card_position = (525, 30)

    def _move_fixed_dealer_card(self, x, y):
        """Move the persistent card without recreating or collecting its items."""
        self._render_fixed_dealer_card()
        old_x, old_y = self._fixed_dealer_card_position
        self.canvas.move('fixed_dealer_card', x - old_x, y - old_y)
        self._fixed_dealer_card_position = (x, y)
    ...
    def _draw_dealer_total_badge(self):
        ...
        left = self._dealer_card_target(0)[0]
        right = self._dealer_card_target(len(self.dealer_cards) - 1)[0] + 100
        mid_y = 100.0
    ...
    def render_cards(self):
        ...
        self._render_fixed_dealer_card()
        self._move_fixed_dealer_card(*self._dealer_card_target(0))
        self.canvas.tag_raise('fixed_dealer_card')
        for i, card in enumerate(self.dealer_cards):
            if i == 0 and card == self.DEALER_FIXED_CARD:
                continue
    ...
    def _begin_round_after_bet_layout(self):
        ...
        self.split_actions=0; self.dealer_cards=[self.DEALER_FIXED_CARD]; self.round_deal_sequence=[]; self.side_bet_results={}
        ...
        # Fixed six is already visible; only the player's two cards consume CSM cards.
    ...
    def _after_initial_deal(self):
        ...
        if BlackjackEngine.is_blackjack(self.hands[0]['cards']):
            self._close_simple_bust_offer()
            self._continue_after_simple_bust()
            return
        self.simple_bust_offer_open = True
        self.canvas.itemconfigure(self.phase_text, text=self.PHASE_PLAYING)
        self.render_cards(); self.update_display()

    def _close_simple_bust_offer(self):
        # A valid first player action implicitly declines an unpurchased wager.
        self.simple_bust_offer_open = False
        self.canvas.delete('simple_bust_options')
        self.canvas.configure(cursor='')

    def _draw_simple_bust_options(self):
        """Four green chip callouts above the wager, with inward-facing pointers."""
        self.canvas.delete('simple_bust_options')
        if (not self.simple_bust_offer_open or not self.round_active
                or self.animation_running or self.settlement_running):
            return
        x0, y0, x1, y1 = self.bet_spots['Simple Bust']['bounds']
        colors = ('#c6efb3', '#a0df92', '#70bf6b', '#409d50')
        radius = 22
        cy = y0 - 32
        target_x, target_y = (x0 + x1) / 2, (y0 + y1) / 2
        centers = [x0 + 13 + i * 47 for i in range(4)]
        for index, (fraction, color, cx) in enumerate(zip(
                self.SIMPLE_BUST_FRACTIONS, colors, centers)):
            amount = round(self.simple_bust_limit() * fraction, 2)
            enabled = self.balance + 1e-9 >= amount
            tag = f'simple_bust_option_top_{index}'
            tags = ('simple_bust_options', tag)
            # Rotate each pointer toward the oval center instead of straight down.
            dx, dy = target_x - cx, target_y - cy
            length = math.hypot(dx, dy)
            ux, uy = dx / length, dy / length
            bx, by = cx + ux * 15, cy + uy * 15
            self.canvas.create_polygon(
                bx - uy * 6, by + ux * 6,
                bx + uy * 6, by - ux * 6,
                cx + ux * 31, cy + uy * 31,
                fill=color, outline='', tags=tags)
            # Preserve the green callout, but use the game's exact chip UI,
            # denomination colors and compact numeric labels without overrides.
            self.canvas.create_oval(
                cx-radius, cy-radius, cx+radius, cy+radius,
                fill=color, outline='#e8f5df', width=1, tags=tags)
            self._draw_baccarat_chip(cx, cy, amount, tags=tags, radius=20)
            self.canvas.create_text(
                cx, cy-radius-10, text=self._compact_amount(amount),
                font=(self.cn_font, 10, 'bold'),
                fill='#eef8e8' if enabled else '#839888', tags=tags)
            self.canvas.tag_bind(
                tag, '<Button-1>', lambda _e, f=fraction: self._choose_simple_bust(f))
            self.canvas.tag_bind(
                tag, '<Enter>', lambda _e, ok=enabled: self.canvas.configure(
                    cursor='hand2' if ok else ''))
            self.canvas.tag_bind(
                tag, '<Leave>', lambda _e: self.canvas.configure(cursor=''))
        self.canvas.tag_raise('simple_bust_options')

    def simple_bust_limit(self):
        """Original main wager, unaffected by subsequent split/double actions."""
        return max(0.0, min(self.MAX_SIDE_BET, self.round_original_bet))

    def _choose_simple_bust(self, fraction):
        if (not self.round_active or self.animation_running or self.settlement_running
                or not self.simple_bust_offer_open
                or fraction not in self.SIMPLE_BUST_FRACTIONS + (0.0,)):
            return
        amount = round(self.simple_bust_limit() * fraction, 2)
        if self.balance + 1e-9 < amount:
            return  # Unaffordable fixed options are disabled; never silently reduce them.
        self._close_simple_bust_offer()  # Lock before balance changes or callbacks.
        self.round_side_bets['Simple Bust'] = amount
        self.balance -= amount
        self.show_last_win = False
        self.save_balance()
        self._continue_after_simple_bust()

    def _continue_after_simple_bust(self):
        hand=self.hands[0]
    ...
    def can_hit(self, hand):
        return bool(not self.ace_decision_mode and hand and hand['status'] == 'active' and not hand.get('split_aces', False))

    def can_double(self, hand):
        return bool(
            not self.ace_decision_mode and hand and hand['status'] == 'active' and len(hand['cards']) == 2
            and not hand.get('split_aces', False)
            and self.balance + 1e-9 >= hand['base_bet']
        )

    def can_split(self, hand):
        if self.ace_decision_mode:
            return False
    ...
    def stand(self):
        hand = self.active_hand()
        if self.ace_decision_mode or not hand or hand['status'] != 'active' or self.animation_running:
            return
        self._close_simple_bust_offer()
        hand['status'] = 'stood'
        hand['result'] = '停牌'
    ...
    def surrender_offer(self, hand):
        ...
        uses 1.61 × outright win probability × current-hand stake.
        """
        ...
        credit=self.hand_stake(hand)*win*(1.0+self.MAIN_PROFIT)
    ...
    @staticmethod
    def _lucky_ladies_result(player_cards):
        """First two player cards only; highest matching tier wins, no stacking."""
        ...
        if p0[1] == p1[1] == 'Q':
            return 20, '两张Q'
        if BlackjackEngine.hand_value([p0, p1])[0] == 20:
            if p0[0] == p1[0]:
                return 7, '同花20点'
            return 3, '任意20点'
        if ('Heart', 'Q') in (p0, p1):
            return 2, '含红心Q'
        if any(card[1] == 'Q' for card in (p0, p1)):
            return 1, '含Q'
        return None, '未中'

    def _settle_initial_side_bets(self):
        if not self.hands or len(self.hands[0]['cards']) < 2:
            return
        ...
        self._credit_side_bet('Lucky Ladies', *self._lucky_ladies_result([p0, p1]))
    ...
    def _maybe_settle_crazy7(self, final=False):
        """Use opening player cards, then the next real dealt card (either side).

        The virtual six consumes no sequence position. If cash-out ends the round
        before a third card, pay the already secured tier without drawing a card.
        """
        key = 'Crazy 7'
        if key in self.side_bet_results or not self.round_deal_sequence:
            return
        seq = self.round_deal_sequence
        c0 = seq[0]
        if c0[1] != '7':
            self._credit_side_bet(key, None, '首张非7', collect_loss=False)
            self.save_balance()
            return
        odds, label = 5, '第一目标7'
        resolved = False
        if len(seq) >= 2:
            ...
            if c1[1] != '7':
                ...
                odds, label = ((150, '前两目标同花7') if c0[0] == c1[0]
                               else (25, '前两目标7'))
                if len(seq) >= 3:
                    c2 = seq[2]
                    if c2[1] == '7':
                        odds, label = ((2000, '目标三张同花7') if c0[0] == c1[0] == c2[0]
                                       else (500, '目标三张7'))
                    resolved = True
    ...
    def _start_dealer_turn(self):
        if (not self.round_active or self.animation_running or self.ace_decision_mode
                or self.simple_bust_offer_open):
            return
        self.animation_running = True
        bust_wager = self.round_side_bets.get('Simple Bust', 0.0) > 0.0
        if not bust_wager and (self._all_hands_bust_or_surrendered() or self._single_natural_blackjack()):
    ...
    def _resolve_normal_round(self):
        self._maybe_settle_crazy7(final=True)
        dealer_total, _ = BlackjackEngine.hand_value(self.dealer_cards)
        dealer_bust = dealer_total > 21
        if self.round_side_bets.get('Simple Bust', 0.0) > 0.0:
            self._credit_side_bet('Simple Bust', 1 if dealer_bust else None,
                                  '庄家爆牌' if dealer_bust else '庄家未爆',
                                  collect_loss=False)
        ...
        for idx, hand in enumerate(self.hands):
            ...
            if hand['status'] == 'bust' or player_total > 21:
                ...
                # it pays the ordinary 0.61:1 main-game win.
                credit = stake * (1.0 + self.MAIN_PROFIT)
                self.balance += credit
                hand['settlement'] = 'win'
                hand['base_settlement'] = 'win'
                hand['double_settlement'] = 'win' if hand.get('double_added', 0.0) > 0 else None
                ...
                if split_two_card_21:
                    hand['result'] = f'分牌21 胜0.61:1 +{self.format_money(credit - stake)}'
                    lines.append(f'手{idx + 1}：分牌21 胜0.61:1')
                else:
                    hand['result'] = f'胜0.61:1 +{self.format_money(credit - stake)}'
    ...
    def _reset_after_round(self):
        ...
        self.hands=[]; self.dealer_cards=[self.DEALER_FIXED_CARD]; self.active_hand_index=0; self.split_actions=0
    ...
    def update_display(self):
        ...
        self._draw_simple_bust_options()
    ...
    def show_game_instructions(self):
        ...
        tk.Label(
            header, text='永6黑杰克  ALWAYS 6 BLACKJACK',
        ...
        add_rule(
            ...
            '普通获胜0.61:1；未分牌的两张天然黑杰克按3:2支付。')
        add_rule(
            left_rules, '庄家永6',
            '庄家第一张为固定6点（Always6.png），不从牌靴抽取；开局只发两张牌给玩家。'
            '玩家行动结束后，庄家从6点开始补牌至17点或以上，软17也停牌。')
        add_rule(
            left_rules, '分牌 / 加倍',
        ...
        def action_icon(cv, kind, x, y, color):
            if kind == 'hit':
                ...
                draw_small_card(cv, x + 2, y + 5, '6', '')
                cv.create_text(x + 69, y + 32, text='固定',
        ...
        draw_action_tile(470, 246, '庄家永6',
                         '固定6点起手；硬17、软17均停牌。', '#4e4030', 'always6')
        ...
        for x0, y0, x1, y1, label in side_spots:
            ...
            bet_cv.create_text(
                (x0 + x1) / 2, (y0 + y1) / 2 + 15,
                text='MAX $100K',
                font=(font_name, FONT_SMALL), fill=GOLD)

        tk.Label(
            bet_card,
            text='主注最低 $100、最高 $500,000；开局前三项边注各自最高 $100,000，可右键清除；简单爆牌在首两张后另行选择。',
            font=(font_name, FONT_SMALL), fg=MUTED, bg=PANEL,
        ...
        # 05 边注赔付
        # ================================================================
        side_card = make_card('05  边注赔付表', '四个边注 · 按利润赔率结算')
        ...
        add_rule(
            notes_card, '简单爆牌下注时机与限额',
            '玩家首两张牌发完后，简单爆牌上方显示四个筹码按钮，上方文字为实际下注金额，尖嘴朝向下注区；仅可下注一次。'
            '上限为100,000与本局原始主注金额中的较小值；由浅绿到深绿对应25%、50%、75%、100%。'
            '玩家执行要牌、停牌、加倍、分牌或兑现时，若尚未下注即视为放弃；玩家首两张为天然黑杰克时，自动跳过简单爆牌，直接结算主注。分牌及加倍不提高上限，余额不足的档位不可选，重复下注不会自动买入。')
        add_rule(
            notes_card, '简单爆牌结算',
            '庄家不论多少张牌爆牌，净利润均为1:1，并返还本金；未爆牌则输掉边注。'
            '已下注时，即使玩家全部爆牌或兑现，庄家仍会补牌至17点或以上或爆牌。')
        add_rule(
            notes_card, '幸运女士',
            '仅按玩家首两张牌判定；按赔付表从高到低取一个最高档，不叠加。A可按11点计算。')
        add_rule(
            notes_card, '疯狂7取牌位置',
            '疯狂7取玩家首两张牌及之后第一张实际发出的牌（玩家或庄家）；固定6点不计入。首张为7先保证5:1；'
            '后续目标7可升级到25/150，再升级到500/2000。若提前兑现且未发第三张，按已锁定档结算。')
        add_rule(
            notes_card, '分牌行动顺序',
