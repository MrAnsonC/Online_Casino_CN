# Evolution Lightning Blackjack — ENHC/OBO house variant
import json
import math
import os
import random
...
BLACKJACK_VERSION = "LIGHTNING-V1"
...
class BubbleBlackjackGame(tk.Frame):
    ...
    MAX_SIDE_BET = 0.0
    MAX_SPLIT_ACTIONS = 1       # user rule: at most one split action
    MAX_HANDS = 2
    ...
    SIDE_BET_ODDS_TEXT = {
        ...
        'Open Card Luck': '80/8/4/3:1',
        'Crazy 7': '2000/500/150/25/5:1',
    ...
    PHASE_BETTING = '闪电黑杰克 · 请下注'
    PHASE_INSURANCE = '闪电黑杰克 · 保险？'
    PHASE_PLAYING = '闪电黑杰克 · 游戏中'
    PHASE_SETTLING = '闪电黑杰克 · 结算中'

    # User-specified multiplier values.
    LIGHTNING_MULTIPLIERS = {
        17: (2,), 18: (2, 3, 4), 19: (3, 4, 5, 6),
        20: (4, 5, 6, 8), 21: (5, 6, 8, 10, 12),
        'blackjack': (6, 8, 12, 15, 20),
    }
    ...
    def __init__(self, parent, balance=10000, user=None, on_back=None,
        ...
        self.username = user
        self.interface_mode = bool(user)
        self.persist_player_carryover = False
        ...
        self.lightning_multiplier = 1
        self.lightning_coverage = 0.0
        self.next_lightning_multiplier = 1
        self.next_lightning_coverage = 0.0
        self.lightning_fee = 0.0
        self.lightning_fee_collecting = False
        self.lightning_offer = self._roll_lightning_offers()
        self.lightning_highlight_key = None
        ...
        self.pending_fee_animation = 0.0
        self._closing = False
        ...
        self.bet_spots = {}
        self.side_bet_spots = {}
        self.lightning_multiplier_items = {}
        self.after_ids = []
    ...
    def draw_animation_panel(self):
        ...
        self.rule_strip_text = c.create_text(
            (rx0 + rx1) / 2, (ry0 + ry1) / 2,
            text='黑杰克3:2 · 100%额外手续费 · 上局倍数,下局加倍 · 保险2:1',
    ...
    def draw_board(self):
        ...
        self.bet_spots = {}
        self.side_bet_spots = {}
        labels = {'MAIN': '', **self.SIDE_BET_LABELS}
        ...
        # Six multiplier panels are revealed only after the fee is collected.
        # They flank MAIN in two vertical groups, as on the live table.
        multiplier_keys = (17, 18, 19, 20, 21, 'blackjack')
        multiplier_labels = {17: '17或以下', 18: '18', 19: '19', 20: '20',
                             21: '21', 'blackjack': 'BJ'}
        panel_bounds = {
            17: (185, 438, 315, 486), 18: (185, 490, 315, 538), 19: (185, 542, 315, 590),
            20: (835, 438, 965, 486), 21: (835, 490, 965, 538),
            'blackjack': (835, 542, 965, 590),
        }
        self.lightning_multiplier_centers = {}
        for key in multiplier_keys:
            bounds = panel_bounds[key]
            x0, y0, x1, y1 = bounds
            tag = f'lightning_multiplier_{key}'
            rect_item=c.create_rectangle(x0, y0, x1, y1, fill='#281d35', outline='#d9b4ff',
                                         width=2, state='hidden',
                                         tags=('lightning_multiplier_panel', tag))
            label_item=c.create_text(x0+10, (y0+y1)/2, anchor='w', text=multiplier_labels[key],
                                     font=(self.cn_font, 10, 'bold'), fill='#e8dcff',
                                     state='hidden', tags=(tag,))
            value_item = c.create_text(x1-10, (y0+y1)/2, anchor='e',
                                       text=f'{self.lightning_offer[key]}×',
                                       font=(self.cn_font, 17, 'bold'), fill='#ffe66d',
                                       state='hidden', tags=(tag,))
            self.lightning_multiplier_items[key] = value_item
            if not hasattr(self,'lightning_multiplier_parts'): self.lightning_multiplier_parts={}
            self.lightning_multiplier_parts[key]=(rect_item,label_item,value_item)
            self.lightning_multiplier_centers[key] = ((x0+x1)/2, (y0+y1)/2)

        # The extra portion is not a side bet: it is the part of a raised main
        # wager above the amount protected by the current earned multiplier.
        self.extra_bet_bounds = (585.0, 462.0, 685.0, 540.0)
        self.extra_bet_chip_pos = (635.0, 512.0)
        c.create_oval(*self.extra_bet_bounds, fill='#28303a', outline='#91a6ba', width=2,
                      state='hidden', tags=('extra_main_bet_spot',))
        c.tag_bind('extra_main_bet_spot','<Button-1>',lambda _e:self.place_bet())
        c.tag_bind('extra_main_bet_spot','<Button-3>',lambda _e:self.clear_bets())
        c.tag_bind('extra_main_bet_spot','<Enter>',lambda _e:c.configure(cursor='hand2'))
        c.tag_bind('extra_main_bet_spot','<Leave>',lambda _e:c.configure(cursor=''))

        # Persistent earned-multiplier badge on MAIN's upper-left corner.
        bx, by = 472.0, 420.0
        self.lightning_badge_circle = c.create_oval(
            bx-23, by-23, bx+23, by+23, fill='#281d35', outline='#ffe66d', width=3,
            state='hidden', tags=('lightning_main_badge',))
        self.lightning_badge_text = c.create_text(
            bx, by, text='', font=(self.cn_font, 14, 'bold'), fill='#ffe66d',
            state='hidden', tags=('lightning_main_badge',))
    ...
    def _resolve_ace_decision(self, accept):
        ...
        if accept:
            ...
            def done():
                ...
                if hand and BlackjackEngine.is_blackjack(hand.get('cards',())):
    ...
    def _animate_selected_chip_to_position(self, tx, ty, amount, tag, on_complete=None):
        """Fly a selected-denomination chip to an explicit MAIN/EXTRA/FEE point."""
        sx, sy = self._selected_chip_origin(); amount = float(amount)
        items = self._draw_baccarat_chip(sx, sy, amount, tags=(tag,), radius=20)
        steps, frame_ms = 10, 20
        def frame(step=1):
            t=min(1.0,step/float(steps)); e=1.0-(1.0-t)**2
            cx=sx+(tx-sx)*e; cy=sy+(ty-sy)*e; r=20
            self.canvas.coords(items[0],cx-r,cy-r,cx+r,cy+r)
            self.canvas.coords(items[1],cx-r+3,cy-r+3,cx+r-3,cy+r-3)
            self.canvas.coords(items[2],cx,cy)
            if step<steps:
    ...
    def _animate_new_main_wager(self, old_total, amount, on_complete=None):
        """Route a new wager chip to covered MAIN and/or uncovered EXTRA."""
        coverage = (float(self.next_lightning_coverage)
                    if self.next_lightning_multiplier > 1 else float('inf'))
        main_amount = max(0.0, min(old_total + amount, coverage) - min(old_total, coverage))
        extra_amount = max(0.0, amount - main_amount)
        pieces=[]
        if main_amount>0:
            x,y=self.bet_spots['MAIN']['chip_pos']
            pieces.append((x,y,main_amount,'main_bet_fly'))
        if extra_amount>0:
            x,y=self.extra_bet_chip_pos
            pieces.append((x,y,extra_amount,'extra_bet_fly'))
        remaining=[len(pieces)]
        def piece_done():
            remaining[0]-=1
            if remaining[0]<=0 and callable(on_complete): on_complete()
        for x,y,value,tag in pieces:
            self._animate_selected_chip_to_position(x,y,value,tag,piece_done)

    def _animate_new_fee_wager(self, amount, on_complete=None):
        x0,y0,x1,_y1=self.bet_spots['MAIN']['bounds']
        self._animate_selected_chip_to_position(
            x1-18.0,y0+20.0,amount,'fee_bet_fly',on_complete)
    ...
    def update_bet_chips(self):
        ...
        if self.round_active:
            show_extra = bool(self.lightning_coverage > 0
                              and self.round_original_bet > self.lightning_coverage)
        else:
            show_extra = bool(self.next_lightning_multiplier > 1
                              and self.next_lightning_coverage > 0
                              and self.current_bet > self.next_lightning_coverage)
        for key, spot in self.bet_spots.items():
            amount = self._bet_amount_for_spot(key)
            if amount <= 0:
                continue
            x, y = spot['chip_pos']
            if key == 'MAIN' and not self.round_active:
                # Only the portion up to the previous winning wager is covered
                # by an earned multiplier; a raised excess is shown separately.
                covered_limit = (float(self.next_lightning_coverage)
                                 if self.next_lightning_multiplier > 1 else amount)
                covered = min(amount, covered_limit)
                extra = max(0.0, amount - covered)
                if covered > 0:
                    self._draw_baccarat_chip(
                        x, y, covered,
                        tags=('bet_chip_dynamic', spot['tag'], 'covered_main_chip'), radius=20)
                if extra > 0:
                    ex, ey = self.extra_bet_chip_pos
                    self._draw_baccarat_chip(
                        ex, ey, extra, tags=('bet_chip_dynamic', 'extra_main_bet_chip'), radius=20)
            else:
                self._draw_baccarat_chip(
                    x, y, amount, tags=('bet_chip_dynamic', spot['tag']), radius=20)

        # Fee is displayed as its own chip at the upper-right of MAIN before
        # the deal. It is a mandatory charge, not part of either wager portion.
        if (not self.round_active and not self.lightning_fee_collecting
                and self.current_bet - self.pending_fee_animation > 0):
            x0, y0, x1, _y1 = self.bet_spots['MAIN']['bounds']
            fx, fy = x1 - 18.0, y0 + 20.0
            self._draw_baccarat_chip(
                fx, fy, self.current_bet - self.pending_fee_animation,
                tags=('bet_chip_dynamic', 'lightning_fee_chip'), radius=18)
            self.canvas.create_text(
                fx, fy - 27, text='FEE', font=(self.cn_font, 9, 'bold'), fill='#ffcf68',
                tags=('bet_chip_dynamic', 'lightning_fee_chip'))
        self.canvas.itemconfigure('extra_main_bet_spot',
                                  state='normal' if show_extra else 'hidden')
        self._refresh_lightning_badge()
    ...
    def _finish_fee_chip_animation(self, amount):
        self.pending_fee_animation = max(0.0, self.pending_fee_animation - float(amount))
        self.bet_chip_animation_count = max(0, self.bet_chip_animation_count - 1)
        self.update_display()
    ...
    def _settlement_losing_chip_sources(self):
        ...
        extra_layout=self._extra_split_wager_layout(len(self.hands)) if self.hands else []
        for idx,hand in enumerate(self.hands):
            if idx>=len(layout): break
            for component in ('base','double'):
                state=hand.get('base_settlement' if component=='base' else 'double_settlement')
                if state!='lose': continue
                covered,extra=self._wager_portions(hand,component)
                if covered>0:
                    x,y=layout[idx][component]; sources.append((x,y,covered,20))
                if extra>0:
                    x,y=extra_layout[idx][component]; sources.append((x,y,extra,18))
    ...
    def _settlement_return_chip_sources(self):
        ...
        for idx,hand in enumerate(self.hands):
            if idx>=len(layout): break
            for component in ('base','double'):
                covered=self._portion_return_amount(hand,component,True)
                extra=self._portion_return_amount(hand,component,False)
                if covered>0:
                    x,y=layout[idx][component]; sources.append((x,y,covered,20))
                if extra>0:
                    x,y=extra_layout[idx][component]; sources.append((x,y,extra,18))
    ...
    def _finish_flash_with_chip_return(self):
        ...
        def done():
            self.settlement_running=False
            self._animate_awarded_multiplier_into_main(self._reset_after_round)
    ...
    def place_bet(self):
        ...
        old_total = float(self.current_bet)
        self.balance -= amount
        self.current_bet += amount
        self.pending_bet_animation['MAIN'] += amount
        self.pending_fee_animation += amount
        self.bet_chip_animation_count += 2
        self.update_display()
        finish_main=lambda:self._finish_bet_chip_animation('MAIN',amount)
        self._animate_new_main_wager(old_total,amount,finish_main)
        self._animate_new_fee_wager(amount, lambda: self._finish_fee_chip_animation(amount))
    ...
    def repeat_last_bet(self):
        ...
        self.show_last_win = False
        main_old_total = float(self.current_bet)
        ...
        for key, add in additions.items():
            ...
            if key == 'MAIN':
                ...
                self.pending_fee_animation += add
            else:
                self.current_side_bets[key] += add
            self.pending_bet_animation[key] += add
            self.bet_chip_animation_count += 2 if key == 'MAIN' else 1
        ...
        for key, add in additions.items():
            if add <= 0.0:
                continue
            if key == 'MAIN':
                self._animate_new_main_wager(
                    main_old_total, add,
                    lambda k=key,a=add:self._finish_bet_chip_animation(k,a))
                self._animate_new_fee_wager(
                    add, lambda a=add:self._finish_fee_chip_animation(a))
            else:
    ...
    def save_runtime_store(self, burn_complete=True):
        ...
        # Interface mode only: append the one-use carry-over record after the
        # shoe data. Standalone launches intentionally omit this section.
        if self.interface_mode and self.persist_player_carryover:
            data['player_last_round'] = {
                'username': self.username,
                'last_multiplier': int(self.next_lightning_multiplier),
                'last_bet': float(self.next_lightning_coverage),
            }
    ...
    def load_runtime_store(self):
        try:
            ...
            if self.interface_mode:
                carry=data.get('player_last_round')
                if isinstance(carry,dict) and carry.get('username')==self.username:
                    multiplier=int(carry.get('last_multiplier',1))
                    last_bet=float(carry.get('last_bet',0.0))
                    if multiplier>=1 and last_bet>=0:
                        self.next_lightning_multiplier=multiplier
                        self.next_lightning_coverage=last_bet
                        self.persist_player_carryover=True
            return True
    ...
    def _component_return_amount(self, hand, component):
        ...
        if amount <= 0.0 or not state:
            return 0.0
        if state == 'win':
            # Allocate the exact (possibly multiplied) return across displayed
            # base/double chips, so the animation agrees with the credited cash.
    ...
    def _wager_portions(self, hand, component):
        """Return (covered MAIN, ordinary EXTRA) for base or double."""
        base=float(hand.get('base_bet',0.0)); dbl=float(hand.get('double_added',0.0))
        covered_base=min(base,float(hand.get('base_main_amount',base)))
        covered_double=min(dbl,float(hand.get('double_main_amount',dbl)))
        amount,covered=(base,covered_base) if component=='base' else (dbl,covered_double)
        return covered,max(0.0,amount-covered)

    def _portion_return_amount(self, hand, component, covered_portion):
        covered,extra=self._wager_portions(hand,component)
        amount=covered if covered_portion else extra
        state=hand.get('base_settlement' if component=='base' else 'double_settlement')
        if amount<=0 or not state: return 0.0
        if state=='win':
            natural=(component=='base' and not hand.get('from_split',False)
                     and BlackjackEngine.is_blackjack(hand.get('cards',())))
            rate=(1.0+self.BLACKJACK_PROFIT) if natural else 2.0
            if covered_portion:
                return amount + amount*(rate-1.0)*self.lightning_multiplier
            return amount*rate
        if state in ('push','refund'): return amount
        return 0.0

    def _extra_split_wager_layout(self, count):
        n=max(1,min(2,int(count)))
        xs=(635.0,) if n==1 else (612.0,658.0)
        return [{'base':(x,505.0),'double':(x,532.0)} for x in xs]
    ...
    def _draw_main_hand_wagers(self):
        ...
        extra_layout = self._extra_split_wager_layout(len(self.hands))
        for idx, hand in enumerate(self.hands):
            pos = layout[idx]
            extra_pos = extra_layout[idx]
            ...
            covered_base,extra_base=self._wager_portions(hand,'base')
            covered_double,extra_double=self._wager_portions(hand,'double')
            if self.settlement_running and self.flash_mode == 'win':
                covered_base=self._portion_return_amount(hand,'base',True)
                extra_base=self._portion_return_amount(hand,'base',False)
                covered_double=self._portion_return_amount(hand,'double',True)
                extra_double=self._portion_return_amount(hand,'double',False)
            ...
            if show_double and covered_double > 0.0:
                self._draw_baccarat_chip(
                    pos['double'][0], pos['double'][1], covered_double,
                    tags=('hand_wager', f'hand_wager_{idx}', 'double_wager'), radius=20)
            if show_double and extra_double > 0.0:
                self._draw_baccarat_chip(extra_pos['double'][0],extra_pos['double'][1],extra_double,
                    tags=('hand_wager',f'hand_wager_{idx}','extra_double_wager'),radius=18)
            if show_base and covered_base > 0.0:
                self._draw_baccarat_chip(
                    pos['base'][0], pos['base'][1], covered_base,
                    tags=('hand_wager', f'hand_wager_{idx}', 'base_wager'), radius=20)
            if show_base and extra_base > 0.0:
                self._draw_baccarat_chip(extra_pos['base'][0],extra_pos['base'][1],extra_base,
                    tags=('hand_wager',f'hand_wager_{idx}','extra_base_wager'),radius=18)
    ...
    def new_hand(self, cards=None, base_bet=0.0, contains_original=False,
                 ...
                 lightning_eligible=0.0, main_amount=None):
        if main_amount is None:
            main_amount = lightning_eligible
        return {
            'cards': list(cards or []),
            'base_bet': float(base_bet),
            'double_added': 0.0,
            'lightning_eligible': float(lightning_eligible),
            'base_lightning_eligible': float(lightning_eligible),
            'double_lightning_eligible': 0.0,
            'base_main_amount': float(main_amount),
            'double_main_amount': 0.0,
    ...
    @classmethod
    def _roll_lightning_offers(cls):
        rng = random.SystemRandom()
        keys=(17,18,19,20,21,'blackjack')
        # Choose only tables satisfying 17<=18<=19<=20<=21<=Blackjack.
        valid=[]
        def build(index,previous,row):
            if index==len(keys): valid.append(tuple(row)); return
            for value in cls.LIGHTNING_MULTIPLIERS[keys[index]]:
                if value>=previous: build(index+1,value,row+[value])
        build(0,0,[])
        chosen=rng.choice(valid)
        return dict(zip(keys,chosen))

    def _refresh_lightning_multiplier_grid(self):
        for key, item in getattr(self, 'lightning_multiplier_items', {}).items():
            self.canvas.itemconfigure(item, text=f'{self.lightning_offer[key]}×')

    def _refresh_lightning_hand_highlight(self, hand=None):
        """Highlight the multiplier bracket matching the current active hand."""
        parts=getattr(self,'lightning_multiplier_parts',{})
        for rect,label,value in parts.values():
            self.canvas.itemconfigure(rect,fill='#281d35',outline='#d9b4ff')
            self.canvas.itemconfigure(label,fill='#e8dcff')
            self.canvas.itemconfigure(value,fill='#ffe66d')
        if self.round_active and not self.settlement_running:
            alive=[]
            for candidate in self.hands:
                cards=candidate.get('cards',())
                if not cards: continue
                total,_soft=BlackjackEngine.hand_value(cards)
                if total<=21: alive.append((total,candidate))
            if not alive:
                self.lightning_highlight_key=None
            else:
                total,candidate=max(alive,key=lambda pair:pair[0])
                if (not candidate.get('from_split',False)
                        and BlackjackEngine.is_blackjack(candidate.get('cards',()))):
                    self.lightning_highlight_key='blackjack'
                else:
                    self.lightning_highlight_key=17 if total<=17 else total
        key=self.lightning_highlight_key if self.round_active else None
        if key in parts:
            rect,label,value=parts[key]
            self.canvas.itemconfigure(rect,fill='#d9b4ff',outline='#f0ddff')
            self.canvas.itemconfigure(label,fill='#111111')
            self.canvas.itemconfigure(value,fill='#7a1620')

    def _set_lightning_grid_visible(self, visible):
        self.canvas.itemconfigure('lightning_multiplier_panel',
                                  state='normal' if visible else 'hidden')
        for key in getattr(self, 'lightning_multiplier_items', {}):
            self.canvas.itemconfigure(f'lightning_multiplier_{key}',
                                      state='normal' if visible else 'hidden')

    def _reveal_lightning_grid(self, on_complete=None):
        """Reveal left/right random multipliers after fee collection."""
        self._refresh_lightning_multiplier_grid()
        keys = (17, 20, 18, 21, 19, 'blackjack')
        def reveal(index=0):
            if index >= len(keys):
                if callable(on_complete): on_complete()
                return
            self.canvas.itemconfigure(f'lightning_multiplier_{keys[index]}', state='normal')
            self._queue(70, reveal, index+1)
        reveal()

    def _refresh_lightning_badge(self):
        if not hasattr(self, 'lightning_badge_circle'):
            return
        x0, y0, _x1, _y1 = self.bet_spots['MAIN']['bounds']
        bx, by = x0 + 17.0, y0 + 18.0
        self.canvas.coords(self.lightning_badge_circle, bx-23, by-23, bx+23, by+23)
        self.canvas.coords(self.lightning_badge_text, bx, by)
        multiplier = (self.lightning_multiplier if self.round_active
                      else self.next_lightning_multiplier)
        visible = int(multiplier) > 1
        state = 'normal' if visible else 'hidden'
        self.canvas.itemconfigure(self.lightning_badge_circle, state=state)
        self.canvas.itemconfigure(self.lightning_badge_text, state=state,
                                  text=f'{int(multiplier)}×' if visible else '')
        if visible:
            self.canvas.tag_raise('lightning_main_badge')

    def _animate_awarded_multiplier_into_main(self, on_complete=None):
        """Move the winning total's displayed multiplier into MAIN's badge."""
        key = getattr(self, 'last_lightning_award_key', None)
        if key is None or self.next_lightning_multiplier <= 1:
            self._set_lightning_grid_visible(False)
            self.lightning_highlight_key=None
            self._refresh_lightning_badge()
            if callable(on_complete): on_complete()
            return
        sx, sy = self.lightning_multiplier_centers[key]
        x0, y0, _x1, _y1 = self.bet_spots['MAIN']['bounds']
        tx, ty = x0 + 17.0, y0 + 18.0
        tag = 'lightning_multiplier_injection'
        self.canvas.delete(tag)
        item = self.canvas.create_text(
            sx, sy, text=f'{self.next_lightning_multiplier}×',
            font=(self.cn_font, 20, 'bold'), fill='#ffe66d', tags=(tag,))
        steps, frame_ms = 16, 22
        def frame(step=1):
            t=min(1.0, step/float(steps)); e=1.0-(1.0-t)**3
            cx=sx+(tx-sx)*e; cy=sy+(ty-sy)*e
            self.canvas.coords(item, cx, cy)
            self.canvas.itemconfigure(item, font=(self.cn_font, int(20-5*e), 'bold'))
            if step < steps:
                self._queue(frame_ms, frame, step+1)
            else:
                self.canvas.delete(tag)
                self._set_lightning_grid_visible(False)
                self.lightning_highlight_key=None
                self.canvas.itemconfigure(self.lightning_badge_circle, state='normal')
                self.canvas.itemconfigure(self.lightning_badge_text, state='normal',
                                          text=f'{int(self.next_lightning_multiplier)}×')
                self.canvas.tag_raise('lightning_main_badge')
                if callable(on_complete): on_complete()
        self._queue(frame_ms, frame, 1)

    def _animate_lightning_fee_to_dealer(self, on_complete=None):
        """Collect the separately displayed fee chip toward the dealer."""
        self.lightning_fee_collecting = True
        self.update_bet_chips()
        x0, y0, x1, _y1 = self.bet_spots['MAIN']['bounds']
        sx, sy = x1 - 18.0, y0 + 20.0
        tx, ty = (self.GAME_X0 + self.GAME_X1) / 2.0, 105.0
        items = self._draw_baccarat_chip(
            sx, sy, self.lightning_fee, tags=('lightning_fee_fly',), radius=18)
        label = self.canvas.create_text(
            sx, sy - 27, text='FEE', font=(self.cn_font, 9, 'bold'), fill='#ffcf68',
            tags=('lightning_fee_fly',))
        steps, frame_ms = 14, 20
        def frame(step=1):
            t = min(1.0, step / float(steps)); e = t*t*(3.0-2.0*t)
            cx, cy = sx+(tx-sx)*e, sy+(ty-sy)*e
            r = 18
            self.canvas.coords(items[0], cx-r, cy-r, cx+r, cy+r)
            self.canvas.coords(items[1], cx-r+3, cy-r+3, cx+r-3, cy+r-3)
            self.canvas.coords(items[2], cx, cy)
            self.canvas.coords(label, cx, cy-27)
            if step < steps:
                self._queue(frame_ms, frame, step+1)
            else:
                self.canvas.delete('lightning_fee_fly')
                if callable(on_complete): on_complete()
        self._queue(frame_ms, frame, 1)
    ...
    def deal_cards(self):
        ...
        # The Lightning Fee equals the main bet and is charged once the deal is
        # committed. It is never returned, including on a dealer Blackjack.
        if self.balance + 1e-9 < self.current_bet:
            messagebox.showwarning('余额不足', '还需要支付与主注等额的 Lightning Fee。',
                                   parent=self.winfo_toplevel()); return
        if self.engine.needs_shuffle() or self.engine.remaining_cards() < 30:
            self.start_new_shoe_cut(); return

        # The loaded carry-over is one-use. Erase it from JSON immediately when
        # the player commits to dealing, while keeping the in-memory values for
        # this round.
        if self.interface_mode and self.persist_player_carryover:
            self.persist_player_carryover=False
            self.save_runtime_store(burn_complete=True)
        ...
        self.lightning_fee = float(self.current_bet)
        self.balance -= self.lightning_fee
        self.save_balance()
        ...
        def after_layout():
            # The six values are generated only after the fee has been taken,
            # immediately before their reveal and before the first card.
            self.lightning_offer = self._roll_lightning_offers()
            self._reveal_lightning_grid(self._begin_round_after_bet_layout)
        def after_fee():
            self._animate_betting_station(True, on_complete=after_layout)
        def after_old_cards():
            self._animate_lightning_fee_to_dealer(after_fee)
    ...
    def _begin_round_after_bet_layout(self):
        ...
        self.dealer_phase_active=False
        self.flash_mode=None
        self.lightning_multiplier = int(self.next_lightning_multiplier)
        self.lightning_coverage = min(float(self.round_original_bet),
                                      float(self.next_lightning_coverage))
        self.next_lightning_multiplier = 1
        self.next_lightning_coverage = 0.0
        main_amount=(self.round_original_bet if self.lightning_multiplier<=1
                     else self.lightning_coverage)
        self.hands=[self.new_hand(base_bet=self.round_original_bet,contains_original=True,
                                  lightning_eligible=self.lightning_coverage,
    ...
    def double(self):
        ...
        # Evolution rule: on a double, the earned multiplier covers the entire
        # doubled win (subject to the original covered portion).
        hand['lightning_eligible'] = min(self.hand_stake(hand),
                                         float(hand.get('lightning_eligible', 0.0)) * 2.0)
        hand['double_lightning_eligible'] = min(
            float(hand.get('double_added',0.0)),
            float(hand.get('base_lightning_eligible',0.0)))
        hand['double_main_amount'] = min(
            float(hand.get('double_added',0.0)),float(hand.get('base_main_amount',0.0)))
    ...
    def split(self):
        ...
        left = self.new_hand(
            ...
            lightning_eligible=hand.get('lightning_eligible', 0.0),
            main_amount=hand.get('base_main_amount',hand['base_bet']))
        right = self.new_hand(
    ...
    def _start_dealer_turn(self):
        ...
        has_supplemental = any(
            (not h.get('contains_original', False)) or h.get('double_added', 0.0) > 0
            for h in self.hands)
        up_rank = self.dealer_cards[0][1] if self.dealer_cards else None

        # V18 fast paths apply only when Bust! was NOT wagered.
        if not bust_wager:
            if all_dead and self.insurance_bet <= 0.0 and not has_supplemental:
        ...
        def after_second():
            dealer_bj = BlackjackEngine.is_blackjack(self.dealer_cards[:2])
            if not bust_wager:
                if all_dead and has_supplemental:
                    self._queue(260, self._resolve_dealer_blackjack
                                if dealer_bj else self._resolve_normal_round)
                    return
    ...
    def _resolve_normal_round(self):
        ...
        for idx, hand in enumerate(self.hands):
            ...
            if hand['status'] == 'bust' or player_total > 21:
                ...
                eligible = min(stake, float(hand.get('lightning_eligible', 0.0)))
                extra = max(0.0, stake - eligible)
                # Principal is returned once; Lightning multiplies profit only.
                credit = (eligible + eligible*self.BLACKJACK_PROFIT*self.lightning_multiplier
                          + extra * (1.0 + self.BLACKJACK_PROFIT))
                self.balance += credit
                hand['settlement'] = 'win'
                ...
                eligible = min(stake, float(hand.get('lightning_eligible', 0.0)))
                extra = max(0.0, stake - eligible)
                credit = (eligible + eligible*self.lightning_multiplier
                          + extra*2.0)
                self.balance += credit
                hand['settlement'] = 'win'
                hand['base_settlement'] = 'win'
    ...
    def _calculate_last_win_amount(self):
        ...
        PUSH, OBO/refunds, instant cashout, side-bet credits and insurance
    ...
    def _award_next_lightning_multiplier(self):
        """Award one next-round multiplier from the highest winning hand."""
        if self.interface_mode:
            self.persist_player_carryover=True
        winners = [h for h in self.hands if h.get('settlement') == 'win']
        if not winners:
            self.last_lightning_award_key = None
            self.next_lightning_multiplier = 1
            self.next_lightning_coverage = float(self.round_original_bet)
            return
        # Once at least one hand wins, use the largest total among all surviving
        # (21-or-less) hands. This matches the requested split-hand rule.
        survivors=[h for h in self.hands
                   if h.get('status') not in ('bust','surrendered')
                   and BlackjackEngine.hand_value(h.get('cards',()))[0]<=21]
        candidates=survivors or winners
        natural = any(not h.get('from_split', False)
                      and BlackjackEngine.is_blackjack(h.get('cards', ())) for h in candidates)
        if natural:
            key = 'blackjack'
        else:
            best = max(BlackjackEngine.hand_value(h.get('cards', ()))[0] for h in candidates)
            key = 17 if best <= 17 else min(21, best)
        self.last_lightning_award_key = key
        self.lightning_highlight_key = key
        self.next_lightning_multiplier = int(self.lightning_offer[key])
        # If the next main wager is raised, only this much receives the multiplier.
        self.next_lightning_coverage = float(self.round_original_bet)
    ...
    def _finish_round(self, dealer_bj=False, phase_text=None):
        ...
        self._award_next_lightning_multiplier()
    ...
    def update_display(self):
        if self._closing: return
        if not self.round_active and not self.settlement_running:
            self.canvas.itemconfigure(self.phase_text, text=self.PHASE_BETTING)
        ...
        self._set_button(self.deal_button,can_bet and no_chip_flying
                         and self.current_bet>=self.MIN_BET
                         and self.balance+1e-9>=self.current_bet)
        ...
        hand=self.active_hand()
        self._refresh_lightning_hand_highlight(hand)
    ...
    def show_game_instructions(self):
        ...
        tk.Label(
            header, text='闪电黑杰克  LIGHTNING BLACKJACK',
            font=(font_name, FONT_TITLE, 'bold'), fg=GOLD, bg=HEADER
        ).place(x=24, y=10)
        tk.Label(
            header, text='玩法 · 切牌 · 决策按钮 · 下注区域 · 赔付表',
        ...
        add_rule(
            ...
            '庄家起手只拿一张明牌，庄家回合才拿第二张。若庄家最终为黑杰克，'
            '额外分牌/加倍本金按OBO退回，只有原下注承担庄家黑杰克的损失。')
        add_rule(
            left_rules, '分牌 / 加倍',
            '同点数价值的两张牌可分牌，最多分牌1次（共2手）；允许分牌后加倍（DAS）。'
            '加倍需追加与当前手原注相同金额，只再拿一张牌后自动停牌。')
        add_rule(
            left_rules, '分A规则',
            'A-A可分牌。分A后每手只补一张牌并自动停牌；分牌后的21点只按普通1:1结算。')
        add_rule(
            left_rules, '庄家A与保险',
            '庄家明牌A时可选择购买或不购买保险。保险金额为原下注的一半，中奖利润2:1；'
            '玩家天然黑杰克也使用同一保险选择，不提供1:1的条件。')

        add_rule(
            right_rules, '① 新牌靴',
            '使用8副完整52张牌，共416张。进入新牌靴时重新洗牌。',
            accent='#7cc8aa')
        add_rule(
            right_rules, '② 切牌',
            '切牌窗口内拖动黄色切牌卡选择位置；确认后将两叠牌交换位置。'
            '也可按ENTER让系统在允许范围内随机完成切牌。',
            accent='#7cc8aa')
        add_rule(
            right_rules, '③ 烧牌',
            '切牌后翻开烧牌值牌：A=1；2–9按牌面；10/J/Q/K=10。随后烧掉对应数量的牌。',
            accent='#7cc8aa')
        add_rule(
            right_rules, '④ 自动换新牌靴',
            '剩余牌数进入随机155–180张阈值时，在下一局开始前进入新的洗牌、切牌与烧牌流程。',
            accent='#7cc8aa')
        add_rule(
            right_rules, '⑤ 闪电手续费',
            '每局主注之外收取100%额外手续费。手续费在发牌前由桌面收走；分牌和加倍不再收手续费。',
            accent='#7cc8aa')
        ...
        draw_action_tile(10, 128, '加倍  DOUBLE',
                         '追加同额下注，只拿一张牌后自动停牌。', ORANGE, 'double')
        draw_action_tile(470, 128, '分牌  SPLIT',
                         '同点值两张牌可分牌；最多分牌1次（共2手），允许DAS。', PURPLE, 'split')
        draw_action_tile(10, 246, '闪电倍数  LIGHTNING',
                         '本局获胜可取得对应点数倍数，供下一局主注盈利使用。', CASH, 'double')
        draw_action_tile(470, 246, '庄家明牌A的决策',
                         '购买保险 / 不购买保险；\n玩家天然黑杰克也相同。', '#4e4030', 'ace')
        ...
        side_spots = [
            (40, 74, 225, 140, '17或以下：2×'),
            (40, 172, 225, 238, '18：2/3/4×'),
            (240, 224, 405, 288, '19：3/4/5/6×'),
            (525, 224, 690, 288, '20：4/5/6/8×'),
            (705, 172, 890, 238, '21：5/6/8/10/12×'),
            (705, 74, 890, 140, 'BJ：6/8/12/15/20×'),
        ]
        for x0, y0, x1, y1, label in side_spots:
            ...
            bet_cv.create_text(
                (x0 + x1) / 2, (y0 + y1) / 2 + 15,
                text='下一局盈利倍数', font=(font_name, FONT_SMALL), fill=GOLD)

        tk.Label(
            bet_card,
            text='主注最低 $100、最高 $500,000。每次主注同时产生等额手续费。若本局下注超过上局倍数覆盖额，'
                 '超出部分进入偏注区并按普通赔率结算；主注和偏注点击均会继续增加总下注。',
            font=(font_name, FONT_SMALL), fg=MUTED, bg=PANEL,
        ...
        # 05 闪电倍数
        # ================================================================
        side_card = make_card('05  闪电倍数表', '本局获胜取得倍数，供下一局使用')
        ...
        tables = [
            ('低点数', [('17或以下', '2×'), ('18', '2 / 3 / 4×')]),
            ('中点数', [('19', '3 / 4 / 5 / 6×'), ('20', '4 / 5 / 6 / 8×')]),
            ('高点数', [('21', '5 / 6 / 8 / 10 / 12×'),
                       ('Blackjack', '6 / 8 / 12 / 15 / 20×')]),
            ('生成限制', [('倍数顺序', '17≤18≤19≤20≤21≤BJ'),
                          ('平局或全负', '下局无倍数')]),
        ]
        ...
        add_rule(
            notes_card, '倍数只乘盈利',
            '总返还 = 本金 +（盈利 × 倍数）。例如主注100、普通获胜、10×时返还1100；'
            '主注100、黑杰克、10×时返还1600。偏注始终按普通赔率结算。')
        add_rule(
            notes_card, '点数格高亮',
            '游戏中按所有未爆牌手牌的最高点数高亮对应倍数格。分牌一手爆牌后忽略该手；'
            '高亮持续到结算、筹码返回并完成倍数注入。')
        add_rule(
            notes_card, '分牌行动顺序',
            '只允许分牌一次。分牌后先完成第一手，再进行第二手；只要至少一手获胜，'
            '便从所有未爆牌手牌中选较高点数，取得对应的下一局倍数。')
        add_rule(
            notes_card, '分牌与加倍的金额组成',
            '分牌和加倍会分别复制主注与偏注金额：主注继续享受相同倍数，偏注继续普通结算；'
            '两种操作都不再收取额外 闪电费。')
        add_rule(
            notes_card, '庄家黑杰克与OBO',
            '欧洲无底牌模式下，庄家黑杰克只收走原下注；之后追加的分牌及加倍本金退回。'
            '闪电费永不退还。')
        add_rule(
            notes_card, '跨局倍数',
            '获胜倍数在结算筹码返回后注入主注左上角。'
    ...
    def exit_game(self):
        ...
        # already-settled cashouts are not refunded again.
        if not self.round_active:
