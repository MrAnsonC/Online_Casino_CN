...
def uth_log_path() -> str:
    ...
    return os.path.join(log_dir, "Ultimate_Texas_Holdem.json")
...
def save_uth_history(deck, player_cards, dealer_cards, community_cards, result_info=None):
    """
    记录当前牌局历史，包含 game_id、统计信息，并只保留最新50局。
    
    参数：
        deck: Deck 实例
        player_cards, dealer_cards, community_cards: Card 列表
        result_info: 字典，可包含 'winner'（'player'/'dealer'/'push'）或 'fold'（弃牌标识）
    """
    ...
    # 解析结果，确定 winner 并更新统计
    winner = None
    if result_info:
        # 优先处理弃牌（fold）
        if result_info.get("fold", False):
            winner = "fold"
            stats["fold"] += 1          # 仅增加弃牌计数，不影响 dealer_win
...
class UTHGUI(tk.Tk):
    ...
    def _create_widgets(self):
        ...
        # 左侧牌桌区域（使用 Canvas 作为背景，内部放置 Frame）
        ...
        # 使用网格布局，使各控件对齐
        body_combined.columnconfigure(0, weight=1)  # 标签列
        body_combined.columnconfigure(1, weight=2)  # 输入框列
        body_combined.columnconfigure(2, weight=1)  # 注释/说明列
        ...
        inner_btn_frame.pack(anchor='center')  # 使内部框架居中
        ...
        # 存放动态生成的游戏动作按钮的容器（指向 body_action）
    ...
    def settle_side_bets_after_fold(self):
        # 结算边注（Trips, Player Pair, Progressive）
