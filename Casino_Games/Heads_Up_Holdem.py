...
def save_progressive(jackpot):
    ...
    if not found:
        data.append({"Games": "UTH_&_UOH_&_CHE_&_HUH", "jackpot": jackpot})
...
class HUHGame:
    ...
    def reset_game(self):
        ...
        self.dealer_hole = []
        self.ante = 0
        self.blind = 0          # 在 Heads-Up 中，blind 代表“赔率注”（Odd）
        self.trips = 0
        self.player_pair = 0
        self.play_bet = 0
...
class HUHGUI(tk.Tk):
    ...
    def show_game_instructions(self):
        ...
        headers = ["牌型", "盲注(玩家赢)", "盲注(玩家输)", "三条注", "累进大奖#"]
        odds_data = [
            ("皇家同花顺", "500:1", "-", "50:1", "100%累进大奖/$100,000"),
            ("同花顺", "50:1", "500:1", "40:1", "10%累进大奖/$10,000"),
            ("四条", "10:1", "50:1", "30:1", "$3,000"),
            ("葫芦", "3:1", "10:1", "8:1", "$500"),
            ("同花", "1.5:1", "6:1", "6:1", "$350"),
            ("顺子", "1:1", "5:1", "5:1", "-"),
            ("三条", "退还", "-","3:1", "-"),
            ("其他牌型", "退还", "-", "-", "-")
        ]
    ...
    def add_chip_to_bet(self, bet_type):
        ...
        if bet_type == "ante":
            ...
            blind_max_bet = 10000  # 赔率注最大下注限制
            
            if blind_current > blind_max_bet:
                # 如果赔率注超过上限，调整Ante和赔率注
    ...
    def reset_single_bet(self, bet_type, event):
        ...
        if bet_type == "ante":
            self.ante_var.set("0")
            self.blind_var.set("0")  # 赔率注自动等于Ante
        elif bet_type == "trips":
    ...
    def cycle_bet_amount(self, event):
        """循环设置Bet下注金额：0 -> 3倍Ante -> 0（Heads-Up 只允许3倍）"""
        # 获取当前Ante金额
        try:
    ...
    def _create_widgets(self):
        ...
        # 赔率注 文本标签
        blind_label = tk.Label(ante_blind_frame, text=": 赔率注", font=('Arial', 14), bg='#2a4a3c', fg='white')
    ...
    def fold_action(self):
        ...
        # 等待1秒后结算Trips、Player Pair、Progressive 和 Odd（赔率注）
        self.after(1000, self.settle_side_bets_after_fold)

    def settle_side_bets_after_fold(self):
        """弃牌后结算Trips、Player Pair、Progressive 和 Odd（赔率注）"""
        # 评估玩家手牌
        ...
        # 结算Odd（赔率注） - 弃牌时直接输掉
        odd_winnings = 0
        if self.game.blind > 0:
            odd_winnings = 0   # 弃牌则 Odd 注全部输掉
        
        # 计算总赢取金额
        total_winnings = trips_winnings + player_pair_winnings + progressive_winnings + odd_winnings
    ...
    def calculate_winnings(self, player_eval, dealer_eval):
        ...
        if player_eval > dealer_eval:
            ...
            if player_rank in player_win_odds:
                odds = player_win_odds[player_rank]
                self.win_details['blind'] = odd_bet * (1 + odds)
            else:
                # 其他牌型（三条及以下）视为平局，退还本金
                self.win_details['blind'] = odd_bet
        elif player_eval == dealer_eval:
            # 平局 -> 退还本金
            self.win_details['blind'] = odd_bet
        else:
            # 庄家赢 -> 根据玩家手牌牌型赔付（庄家赢的赔率表）
            player_rank = player_eval[0]
            dealer_win_odds = {
                8: 500,    # 同花顺
                7: 50,   # 四条
    ...
    def reset_bets(self):
        """重置所有投注金额为0"""
        self.ante_var.set("0")
        self.blind_var.set("0")  # 赔率注自动等于Ante，所以也要重置
        self.trips_var.set("0")
        self.player_pair_var.set("0")
